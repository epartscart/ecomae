<?php
/**
 * Register Super CP → Portal → API documentation guide.
 * Run: https://www.ecomae.com/epc-api-documentation-cp-setup.php?token=epartscart-deploy-2026
 */
...
function epc_adcs_lang(PDO $pdo, string $key, string $en, string $ru): void
{
...
epc_adcs_lang($pdo, 'epc_portal_api_documentation_guide', 'API documentation guide', 'Руководство по API');
$portalGroup = epc_cp_mm_ensure_group($pdo, 'epc_cp_group_portal', 'Portal', 'Портал', 2);
$itemUrl = '/<backend>/control/portal/epc_api_documentation_guide';
$itemId = epc_cp_mm_ensure_item($pdo, $portalGroup, 'epc_portal_api_documentation_guide', $itemUrl, 9, '#7c3aed', 'fa-code', 1);

$contentUrl = 'control/portal/epc_api_documentation_guide';
$phpPath = '/<backend_dir>/content/control/portal/epc_api_documentation_guide.php';
$now = time();
...
if ($contentId > 0) {
	...
	)->execute(array($phpPath, 'epc_portal_api_documentation_guide', 'epc_portal_api_documentation_guide', $parentId, $level, 'epc_api_documentation_guide', $contentId));
} else {
	$pdo->prepare(
		'INSERT INTO `content` (`count`, `url`, `level`, `alias`, `value`, `parent`, `description`, `is_frontend`, `content_type`, `content`,
		 ...
		 VALUES (0, ?, ?, ?, ?, ?, ?, 0, \'php\', ?, ?, \'0\', \'0\', \'0\', 0, \'[]\', \'\', \'\', 0, 1, 0, ?, ?, 9)'
	)->execute(array(
		$contentUrl, $level, 'epc_api_documentation_guide', 'epc_portal_api_documentation_guide', $parentId,
		'Super CP guide — issue tenant API keys, scopes, curl tests',
		$phpPath, 'epc_portal_api_documentation_guide', $now, $now,
	));
	$contentId = (int) $pdo->lastInsertId();
}

$ref = $pdo->prepare('SELECT `id` FROM `content` WHERE `url` = ? AND `is_frontend` = 0 LIMIT 1');
$ref->execute(array('control/portal/epc_erp_only_onboard_guide'));
...
echo "Super CP URL: /cp/control/portal/epc_api_documentation_guide\n";
echo "Marketing: https://www.ecomae.com/platform/api-documentation\n";
echo "OpenAPI: https://www.ecomae.com/epc-api/v1/openapi.json\n";
echo "Done.\n";
