...
$car_caption = htmlentities($_GET["car_caption"], ENT_QUOTES, "UTF-8");
?>
...
function groupChanged()
{
	jQuery.ajax({
		...
		url: "/content/shop/ucats/accessories/ajax_get_group_fields.php",
		dataType: "json",//Тип возвращаемого значения
		data: "car_name="+encodeURI('<?php echo $car_name; ?>')+"&model="+encodeURI('<?php echo $model; ?>')+"&year="+encodeURI('<?php echo $year; ?>'),
...
function showGroupFields(fields)
{
	...
	for(var i=0 ; i < fields.length; i++)
	{
		...
		for(var v=0 ; v < fields[i].values.length; v++)
		{
			...
			if(caption == "") caption = "<?php echo translate_str_by_id(4775); ?>";
	...
	{
		$(group_fields_list[i].id).multipleSelect({placeholder: "<?php echo translate_str_by_id(3200); ?>...", width:"100%", onClose: function(){getProducts()}});
...
function getProducts()
{
	...
	products_query.mark = '<?php echo $car_name; ?>';
	products_query.model = '<?php echo $model; ?>';
	products_query.year = '<?php echo $year; ?>';
	...
	for(var i=0; i < group_fields_list.length; i++)
	{
		...
		{
			alert("<?php echo translate_str_by_id(4776); ?> "+group_fields_list[i].caption);
			return;
		}
	}
	...
	jQuery.ajax({
		type: "POST",
		async: true, //Запрос синхронный
		url: "/content/shop/ucats/accessories/ajax_get_products.php",
		dataType: "json",//Тип возвращаемого значения
...
function showProducts()
{
	...
	for(var i=CurrentPage*Products_per_page; i < Products.length && printed < Products_per_page; i++)
	{
		...
		//Артикул
		html += "<div class=\"article\" onclick=\"location='<?php echo $multilang_params['lang_href']; ?>/shop/part_search?article="+Products[i].article+"';\">"+Products[i].article+"</div>";
		...
		//Кнопка "Подробнее"
		html += "<div class=\"product_page_button\"><a href=\"<?php echo $multilang_params['lang_href']; ?>/shop/katalogi-ucats/avtoaksessuary/vybor-modeli/aksessuary-modeli/tovar?tovar="+Products[i].id+"&car_name=<?php echo urlencode($car_name); ?>&model=<?php echo urlencode($model); ?>&year=<?php echo urlencode($year); ?>&img=<?php echo urlencode($img); ?>&car_caption=<?php echo urlencode($car_caption); ?>\" class=\"bread_crumbs_a\"><?php echo translate_str_by_id(3608); ?></a></div>";
