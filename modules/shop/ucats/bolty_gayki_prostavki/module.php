...
<div style="padding-right:20px;">
	...
	<select id="group_select" onchange="groupChanged();" style="width:100%;" class="form-control">
		<option value="gayki"><?php echo translate_str_by_id(4778); ?></option>
		<option value="bolty"><?php echo translate_str_by_id(4779); ?></option>
		<option value="prostavki"><?php echo translate_str_by_id(4780); ?></option>
	</select>
</div>
...
function groupChanged()
{
	jQuery.ajax({
		...
		url: "/content/shop/ucats/bolty_gayki_prostavki/ajax_get_group_fields.php",
...
function getProducts()
{
	...
	jQuery.ajax({
		type: "POST",
		async: true, //Запрос синхронный
		url: "/content/shop/ucats/bolty_gayki_prostavki/ajax_get_products.php",
		dataType: "json",//Тип возвращаемого значения
...
function showProducts()
{
	...
	for(var i=CurrentPage*Products_per_page; i < Products.length && printed < Products_per_page; i++)
	{
		...
		//Кнопка "Подробнее"
		html += "<div class=\"product_page_button\"><a href=\"<?php echo $multilang_params['lang_href']; ?>/shop/katalogi-ucats/kolesnye-gajki-bolty-prostavki/tovar?group="+document.getElementById("group_select").value+"&tovar="+Products[i].id+"\" class=\"bread_crumbs_a\"><?php echo translate_str_by_id(3608); ?></a></div>";
