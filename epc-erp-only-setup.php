<?php
/**
 * Register Super CP → Portal → ERP-only onboarding guide.
 * Run: https://www.ecomae.com/epc-erp-only-setup.php?token=epartscart-deploy-2026
 */
...
function epc_eos_lang(PDO $pdo, string $key, string $en, string $ru): void
{
...
epc_eos_lang($pdo, 'epc_portal_erp_only_guide', 'ERP-only onboarding', 'ERP без витрины');
epc_eos_lang($pdo, 'epc_erp_only_operator_guide', 'ERP-only operator guide', 'ERP: руководство оператора');

$portalGroup = epc_cp_mm_ensure_group($pdo, 'epc_cp_group_portal', 'Portal', 'Портал', 2);
$itemUrl = '/<backend>/control/portal/epc_erp_only_onboard_guide';
$itemId = epc_cp_mm_ensure_item($pdo, $portalGroup, 'epc_portal_erp_only_guide', $itemUrl, 8, '#0369a1', 'fa-university', 1);

$contentUrl = 'control/portal/epc_erp_only_onboard_guide';
$phpPath = '/<backend_dir>/content/control/portal/epc_erp_only_onboard_guide.php';
$now = time();
...
if ($contentId > 0) {
	...
	)->execute(array($phpPath, 'epc_portal_erp_only_guide', 'epc_portal_erp_only_guide', $parentId, $level, 'epc_erp_only_onboard_guide', $contentId));
} else {
	$pdo->prepare(
		'INSERT INTO `content` (`count`, `url`, `level`, `alias`, `value`, `parent`, `description`, `is_frontend`, `content_type`, `content`,
		 ...
		 VALUES (0, ?, ?, ?, ?, ?, ?, 0, \'php\', ?, ?, \'0\', \'0\', \'0\', 0, \'[]\', \'\', \'\', 0, 1, 0, ?, ?, 8)'
	)->execute(array(
		$contentUrl, $level, 'epc_erp_only_onboard_guide', 'epc_portal_erp_only_guide', $parentId,
		'Super CP guide — ERP-only tenant onboarding (no storefront)',
		$phpPath, 'epc_portal_erp_only_guide', $now, $now,
	));
...
$erpGuideUrl = 'shop/finance/erp/erp-only-operator-guide';
$erpGuidePath = '/<backend_dir>/content/shop/finance/erp/erp_only_operator_guide_page.php';
$erpExisting = $pdo->prepare('SELECT `id` FROM `content` WHERE `url` = ? AND `is_frontend` = 0 LIMIT 1');
$erpExisting->execute(array($erpGuideUrl));
$erpContentId = (int) $erpExisting->fetchColumn();
$erpParent = $pdo->prepare('SELECT `id`, `level` FROM `content` WHERE `url` = ? AND `is_frontend` = 0 LIMIT 1');
$erpParent->execute(array('shop/finance/erp'));
$erpParentRow = $erpParent->fetch(PDO::FETCH_ASSOC);
if ($erpParentRow && $erpContentId <= 0) {
	$pdo->prepare(
		'INSERT INTO `content` (`count`, `url`, `level`, `alias`, `value`, `parent`, `description`, `is_frontend`, `content_type`, `content`,
		 `title_tag`, `description_tag`, `keywords_tag`, `author_tag`, `main_flag`, `modules_array`, `css_js`, `robots_tag`,
		 `system_flag`, `published_flag`, `open`, `time_created`, `time_edited`, `order`)
		 VALUES (0, ?, ?, ?, ?, ?, ?, 0, \'php\', ?, ?, \'0\', \'0\', \'0\', 0, \'[]\', \'\', \'\', 0, 1, 0, ?, ?, 99)'
	)->execute(array(
		$erpGuideUrl, (int) $erpParentRow['level'] + 1, 'erp_only_operator_guide', 'epc_erp_only_operator_guide', (int) $erpParentRow['id'],
		'ERP-only client operator guide',
		$erpGuidePath, 'epc_erp_only_operator_guide', $now, $now,
	));
	$erpContentId = (int) $pdo->lastInsertId();
}
...
echo "ERP operator guide content id: {$erpContentId}\n";
echo 'Route verify: ' . ($ok ? 'ok id=' . $ok['id'] : 'MISSING') . "\n";
echo "Super CP URL: /cp/control/portal/epc_erp_only_onboard_guide\n";
echo "Operator guide: /cp/shop/finance/erp/erp-only-operator-guide?epc_erp_shell=1\n";
echo "Done.\n";
