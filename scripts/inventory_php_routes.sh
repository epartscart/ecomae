...
print(json.dumps({
    ...
    "jobLikeFiles": sum(1 for item in items if item["jobLike"]),
...
for path in "${php_files[@]}"; do
  ...
  if is_job_like "$path"; then job_like_count=$((job_like_count + 1)); fi
done

printf 'Job-like PHP files: %s\n\n' "$job_like_count"
printf '## Surface counts\n\n'
printf '| Surface | PHP files |\n'
printf '| --- | ---: |\n'
for surface in api bos cp erp storefront platform; do
  printf '| %s | %s |\n' "$surface" "${surface_counts["$surface"]:-0}"
done
printf '\n## Inventory\n\n'
