#!/usr/bin/env python3
"""Compare write-dryrun dual-sample contract floor (writes=0).

--contract-only requires php-{stem}.json migration-contract-golden baselines
for every aspnet-* golden derived from the Wave B probe allowlist.
...
def unique_probe_stems(probe_text: str) -> list[str]:
    seen: list[str] = []
    have: set[str] = set()
    for path, _body, _cookie, _label in PROBE_RE.findall(probe_text):
        stem = stem_for(path)
        if stem not in have:
            have.add(stem)
            seen.append(stem)
...
def main() -> int:
    ...
    missing_aspnet: list[str] = []

    if not evidence.is_dir():
        print(f"FAIL: missing evidence dir {evidence}", file=sys.stderr)
        return 1
    if not args.probe.is_file():
        errors.append(f"missing probe {args.probe}")
        probe_stems: list[str] = []
    else:
        probe_stems = unique_probe_stems(args.probe.read_text(encoding="utf-8"))
        if len(probe_stems) < MIN_ACTIONS:
            errors.append(f"probe unique stems={len(probe_stems)} < {MIN_ACTIONS}")
    ...
    if not inv_path.is_file():
        ...
        if not isinstance(actions, list) or len(actions) < MIN_ACTIONS:
            errors.append(
                f"inventory actions count "
                f"{len(actions) if isinstance(actions, list) else 0} < {MIN_ACTIONS}"
            )

    for stem in probe_stems:
        asp = evidence / f"aspnet-{stem}.json"
        if not asp.is_file():
            missing_aspnet.append(stem)
            errors.append(f"missing aspnet-{stem}.json")
            continue
        samples_checked += 1
        doc = load(asp)
        if doc.get("writes") not in (0, None):
            errors.append(f"{asp.name}: writes must be 0")
        if doc.get("cutoverAllowed") is True:
            errors.append(f"{asp.name}: cutoverAllowed must be false")
        if doc.get("readyForPhpRemoval") is True:
            errors.append(f"{asp.name}: readyForPhpRemoval must be false")
        if doc.get("writes") == 0:
            samples_ok += 1
    ...
    if samples_checked < MIN_ACTIONS and not missing_aspnet:
        errors.append(f"samplesChecked={samples_checked} < {MIN_ACTIONS}")

    result = {
        "role": "write-dryrun-dual-sample-compare",
        "generatedAtUnix": int(time.time()),
        "ok": not errors,
        "samplesChecked": samples_checked,
        "samplesOk": samples_ok,
        "probeUniquePaths": len(probe_stems),
        ...
        "missingAspNetSides": missing_aspnet,
        ...
        "note": (
            "Write dry-runs are writes=0. Contract-only pairs use migration-contract-golden "
            "baselines — not live PHP. Live PHP ajax remains authoritative until field "
    ...
    if errors:
        ...
        return 1
    print(
        f"PASS: write-dryrun dual-sample compare cutoverAllowed=false "
        f"contractPairsOk={contract_pairs_ok}/{contract_pairs} -> {args.out}"
    )
    return 0
