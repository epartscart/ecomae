...
expected = 36  # 4 apps + 4 logins + auth/login/admin + orders×2 + users-app + groups-app + modules-app + pages-app + menus-app + tenants-app + currencies-app + storages-app + admin-sessions-app + api-clients-app + config-items-app + audit-log-app + sales-orders-app + purchase-orders-app + invoices-app + cash-accounts-app + coa-accounts-app + gl-journals-app + warehouses-app + suppliers-app + purchases-app + search-app + cart-app + storefront-orders-app + garage-app
