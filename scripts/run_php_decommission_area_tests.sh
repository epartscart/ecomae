...
# Never removes PHP.
# Presentation exact-route inventory (97): docs/migration/evidence/presentation/presentation-exact-routes.json
...
if [[ "${#DIGEST_ROUTES[@]}" -ne 80 ]]; then
  record "surface-digest-route-inventory" fail "expected 80 digest routes, found ${#DIGEST_ROUTES[@]}"
else
