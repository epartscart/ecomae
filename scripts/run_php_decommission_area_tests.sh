...
# Never removes PHP.
# Presentation exact-route inventory (81): docs/migration/evidence/presentation/presentation-exact-routes.json
...
# Live surface digest exact-routes (64/64) expect ASP.NET 401 unauthorized JSON (admin cookie for 200).
...
if [[ "${#DIGEST_ROUTES[@]}" -ne 64 ]]; then
  record "surface-digest-route-inventory" fail "expected 64 digest routes, found ${#DIGEST_ROUTES[@]}"
else
