...
# Never removes PHP.
# Presentation exact-route inventory (73): docs/migration/evidence/presentation/presentation-exact-routes.json
...
# Live surface digest exact-routes (56/56) expect ASP.NET 401 unauthorized JSON (admin cookie for 200).
...
if [[ "${#DIGEST_ROUTES[@]}" -ne 56 ]]; then
  record "surface-digest-route-inventory" fail "expected 56 digest routes, found ${#DIGEST_ROUTES[@]}"
else
