...
# Never removes PHP.
# Presentation exact-route inventory (93): docs/migration/evidence/presentation/presentation-exact-routes.json
...
if [[ "${#DIGEST_ROUTES[@]}" -ne 76 ]]; then
  record "surface-digest-route-inventory" fail "expected 76 digest routes, found ${#DIGEST_ROUTES[@]}"
else
