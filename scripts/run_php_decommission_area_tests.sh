...
# Never removes PHP.
# Presentation exact-route inventory (77): docs/migration/evidence/presentation/presentation-exact-routes.json
...
# Live surface digest exact-routes (60/60) expect ASP.NET 401 unauthorized JSON (admin cookie for 200).
...
if [[ "${#DIGEST_ROUTES[@]}" -ne 60 ]]; then
  record "surface-digest-route-inventory" fail "expected 60 digest routes, found ${#DIGEST_ROUTES[@]}"
else
