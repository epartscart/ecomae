...
# Never removes PHP.
# Presentation exact-route inventory (113): docs/migration/evidence/presentation/presentation-exact-routes.json
...
if [[ "${#DIGEST_ROUTES[@]}" -ne 96 ]]; then
  record "surface-digest-route-inventory" fail "expected 96 digest routes, found ${#DIGEST_ROUTES[@]}"
else
