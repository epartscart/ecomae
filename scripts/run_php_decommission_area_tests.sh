...
if [[ "${#DIGEST_ROUTES[@]}" -ne 129 ]]; then
  record "surface-digest-route-inventory" fail "expected 129 digest routes, found ${#DIGEST_ROUTES[@]}"
else
  record "surface-digest-route-inventory" pass "129 CP/ERP/BOS digest exact-routes in shadow example"
fi
