...
# Never removes PHP.
# Presentation exact-route inventory (61): docs/migration/evidence/presentation/presentation-exact-routes.json
...
# Live surface digest exact-routes (44/44) expect ASP.NET 401 unauthorized JSON (admin cookie for 200).
...
if [[ "${#DIGEST_ROUTES[@]}" -ne 44 ]]; then
  record "surface-digest-route-inventory" fail "expected 44 digest routes, found ${#DIGEST_ROUTES[@]}"
else
