...
# Never removes PHP.
# Presentation exact-route inventory (65): docs/migration/evidence/presentation/presentation-exact-routes.json
...
# Live surface digest exact-routes (48/48) expect ASP.NET 401 unauthorized JSON (admin cookie for 200).
...
if [[ "${#DIGEST_ROUTES[@]}" -ne 48 ]]; then
  record "surface-digest-route-inventory" fail "expected 48 digest routes, found ${#DIGEST_ROUTES[@]}"
else
