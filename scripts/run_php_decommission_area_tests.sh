...
if [[ "${#DIGEST_ROUTES[@]}" -ne 133 ]]; then
  record "surface-digest-route-inventory" fail "expected 133 digest routes, found ${#DIGEST_ROUTES[@]}"
else
  record "surface-digest-route-inventory" pass "133 CP/ERP/BOS digest exact-routes in shadow example"
fi
