#!/usr/bin/env bash
# CloudPanel paste — publish CP Website tracker PHP-parity dashboard (/cp/web-tracker-app).
#
# URL='https://raw.githubusercontent.com/epartscart/ecomae/cursor/cp-web-tracker-php-parity-7b3b/scripts/cloudpanel_EPARTSCART_CP_WEB_TRACKER_NOW.sh'
# TMP=/tmp/cp-web-tracker-now.sh
# curl -fsSL "$URL" -o "$TMP" && test -s "$TMP"
# export ECOMAE_BRANCH=cursor/cp-web-tracker-php-parity-7b3b
# bash "$TMP" 2>&1 | tee /root/cp-web-tracker-parity.log
# grep -E 'RESULT=|PASS |FAIL |SHA=' /root/cp-web-tracker-parity.log | tail -80
set -euo pipefail
ECOMAE_BRANCH="${ECOMAE_BRANCH:-cursor/cp-web-tracker-php-parity-7b3b}"
...
ECOMAE_BRANCH="$ECOMAE_BRANCH" bash scripts/cloudpanel_FORCE_LIVE_NOW.sh 2>&1 | tee /root/cp-web-tracker-force-live.log
fail=0
prove() {
...
prove wt-css 'https://www.epartscart.com/platform-assets/epc_web_tracker_cp.css' 'wt-hero'
prove wt-js 'https://www.epartscart.com/platform-assets/epc_web_tracker_aspnet.js' '/cp/web-tracker/dashboard'
prove wt-app 'https://www.epartscart.com/cp/web-tracker-app' 'Website tracker'
prove wt-dash 'https://www.epartscart.com/cp/web-tracker/dashboard' ''
if [[ "$fail" -eq 0 ]]; then
