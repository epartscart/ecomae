#!/usr/bin/env bash
# Focused CloudPanel paste: bind ePartsCart shop db_name, restore php-reference,
# diagnose CP login for taxofin2025@gmail.com, hard-prove bunches + login error path.
#
#   URL='https://raw.githubusercontent.com/epartscart/ecomae/cursor/cp-login-tenant-db-credentials-7b3b/scripts/cloudpanel_EPARTSCART_BIND_SHOP_DB_NOW.sh'
#   TMP=/tmp/epartscart-bind-shop-db-now.sh
#   curl -fsSL "$URL" -o "$TMP"
#   grep -q BIND_SHOP_DB_NOW "$TMP" || { echo RESULT=FAIL bad_download; exit 1; }
#   export ECOMAE_BRANCH=cursor/cp-login-tenant-db-credentials-7b3b
#   bash "$TMP" 2>&1 | tee /root/epartscart-bind-shop-db-now.log
#   grep -E 'RESULT=|BOUND_|CP_LOGIN_DIAG|GATE_|ERROR|SHA=' /root/epartscart-bind-shop-db-now.log | tail -80
set -euo pipefail
...
grep -q 'discovered_php_config\|SHOW DATABASES LIKE' scripts/cloudpanel_fix_epartscart_portal_tenant_db.sh \
  || die "hardened portal tenant fix missing — wrong tree"
...
grep -q 'RESULT=PASS' /root/epartscart-bind-shop-inner.log || die "portal tenant bind did not PASS"

printf '\n---- [2] restore php-reference ----\n'
set +e
ECOMAE_CONFIRM_RESTORE_PHP_REFERENCE_SERVING=YES \
  bash scripts/cloudpanel_restore_php_reference_serving.sh 2>&1 | tee -a /root/epartscart-bind-shop-inner.log
set -e
...
gate() {
  ...
  local code body
  body="$(mktemp)"
  code="$(curl -sS -o "$body" -w '%{http_code}' --max-time 30 -k -A 'bind-shop' "$url" || echo 000)"
  if [[ "$code" == "200" ]] && grep -qiE "$re" "$body"; then
    printf 'GATE_OK  %s %s\n' "$code" "$url"
  else
    printf 'GATE_BAD %s %s (need ~ %s)\n' "$code" "$url" "$re"
    head -c 180 "$body"; echo
    fail=$((fail + 1))
  fi
  rm -f "$body"
}
...
gate "https://www.epartscart.com/cp/login" "Continue with Google|Shop database is not bound|Incorrect login"
# wrong password should no longer be tenant_db_unbound once shop is bound
...
if [[ "$REDIR" == *'tenant_db_unbound'* ]]; then
  ...
  printf 'GATE_OK  login no longer tenant_db_unbound\n'
fi
...
if [[ "$fail" -gt 0 ]]; then
  die "gates_failed=$fail diag_rc=$DIAG_RC SHA=$SHA — paste /root/epartscart-bind-shop-db-now.log"
fi
