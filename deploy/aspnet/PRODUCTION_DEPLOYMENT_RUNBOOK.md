...
This runs foundation wiring, PHP alias regression, PHP syntax, shell syntax, optional .NET tests, and optional live smoke checks.
...
1. Add the contents of `deploy/aspnet/nginx-diagnostics-only.conf` to the existing site server block.
...
Only after `/migration/readiness`, `/migration/data-parity`, `/migration/cutover-validation`, live smoke, and business validation agree, add an exact route proxy block like `nginx-api-shadow-example.conf`.
