...
If `bash scripts/preflight_aspnet_production.sh` returns `No such file or directory`, the shell is not inside the repository root or the repo has not been cloned to the server yet. Follow `deploy/aspnet/CLOUDPANEL_QUICK_START.md`: find or clone the repo, `cd` into it, then run the preflight.
