<?php
/**
 * Register Super CP / tenant CP → Portal → Power BI.
...
epc_pbi_setup_lang($pdo, 'epc_portal_power_bi', 'Power BI', 'Power BI');
$portalGroup = epc_cp_mm_ensure_group($pdo, 'epc_cp_group_portal', 'Portal', 'Портал', 2);
$itemUrl = '/<backend>/control/portal/epc_power_bi';
$itemId = epc_cp_mm_ensure_item($pdo, $portalGroup, 'epc_portal_power_bi', $itemUrl, 10, '#f2c811', 'fa-bar-chart', 1);

$contentUrl = 'control/portal/epc_power_bi';
$phpPath = '/<backend_dir>/content/control/portal/epc_power_bi.php';
$now = time();
...
if ($contentId > 0) {
	...
	)->execute(array($phpPath, 'epc_portal_power_bi', 'epc_portal_power_bi', $parentId, $level, 'epc_power_bi', $contentId));
} else {
	$pdo->prepare(
		...
		$contentUrl, $level, 'epc_power_bi', 'epc_portal_power_bi', $parentId,
		'Power BI connector — datasets, workspace config, embed',
		$phpPath, 'epc_portal_power_bi', $now, $now,
	));
...
echo "CP URL: /cp/control/portal/epc_power_bi\n";
