...
| Storefront home | `epartscart.com/` | Live `/` = **PHP same-to-same** (classic-entry `rewrite → index.php`). `/storefront/app` is preview only | PHP-live full modex chrome | Digests ASP.NET on `*-app` routes | Do not put Blazor scaffold on live `/` until dual-sample green (header/footer/search/AI/dock) |
