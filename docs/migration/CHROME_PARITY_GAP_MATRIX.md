...
**Same-to-same:** tenants must not feel PHP→ASP.NET. Digests/Blazor previews on www never replace live tenant frontend/CP/ERP/BOS UX. Verify: `bash scripts/cloudpanel_verify_tenant_hosts_still_php.sh`.
...
| Platform CP | `/CP/` | `/cp/app` · orders · users/groups/modules/pages/menus/tenants/currencies/storages/admin-sessions `*-app` | Hybrid: `PhpCpDesktopChrome` + registry/OMS/CMS/portal/finance/logistics/session read UIs | Digests ASP.NET; writes PHP | Content/menu/tenant/currency/storage/user managers still PHP; tenant chrome PHP |
