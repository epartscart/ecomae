...
| Storefront home | `epartscart.com/` | `/storefront/app` · search/cart/orders/garage/profile `*-app` | Hybrid: modex chrome + piston + account-family read UIs | Digests ASP.NET; search/cart/orders/garage/profile read | Live slider/media + qty/guest cart/checkout + order detail + garage/profile edits + part_search tabs stay PHP |
