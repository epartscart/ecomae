...
| Platform CP | `/CP/` | `/cp/app` · orders · users/groups/modules/pages/menus/tenants/currencies/storages/admin-sessions/api-clients `*-app` | Hybrid: `PhpCpDesktopChrome` + registry/OMS/CMS/portal/finance/logistics/session/API read UIs | Digests ASP.NET; writes PHP | Content/menu/tenant/currency/storage/user/API managers still PHP; tenant chrome PHP |
