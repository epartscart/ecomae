...
| Platform CP | `/CP/` | `/cp/app` · orders · users/groups/modules/pages/menus/tenants/currencies/storages `*-app` | Hybrid: `PhpCpDesktopChrome` + registry/OMS/CMS/portal/finance/logistics read UIs | Digests ASP.NET; writes PHP | Content/menu/tenant/currency/storage managers still PHP; tenant chrome PHP |
