...
| Storefront home | `epartscart.com/` | Live `/` = **ASP.NET PHP-style** (`proxy → /storefront/app`). PHP only via `/php-reference/home` | ASP.NET chrome with PHP fingerprints (Garage Manager / WhatsApp / AI / tabs / dock) | Digests ASP.NET on `*-app` routes | Deep module dual-sample still open; `cutoverAllowed=false` |
