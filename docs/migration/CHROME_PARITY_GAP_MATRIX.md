...
| Platform ERP | `/ERP/` | `/erp/app` · `/erp/sales-orders-app` · `/erp/purchase-orders-app` · `/erp/invoices-app` · `/erp/cash-accounts-app` | Hybrid: `PhpErpDesktopChrome` + SO/PO/INV/CASH read UIs | Digests ASP.NET; writes PHP | Full tab UIs / ajax_erp still PHP; tenant chrome PHP |
