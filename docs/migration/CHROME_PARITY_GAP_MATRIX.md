...
| Platform ERP | `/ERP/` | `/erp/app` · `/erp/sales-orders-app` · `/erp/purchase-orders-app` · `/erp/invoices-app` | Hybrid: `PhpErpDesktopChrome` + SO/PO/INV read UIs | Digests ASP.NET; SO/PO/INV writes PHP | Full tab UIs / ajax_erp still PHP; tenant chrome PHP |
