...
- Assign every route/job an owner: ASP.NET Core, deleted, or ASP.NET Core with Python AI-service helper.
- Mark every item as `php-only`, `aspnet-shadow`, `aspnet-with-python-ai-helper`, `parity-ready`, `live`, or `removed`.
...
2. Route inventory owner assignment updates the tracked JSON/Markdown artifact from generated `php-only` baseline to explicit owners and parity states.
