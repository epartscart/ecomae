...
**Live (public unauth ASP.NET JSON 401):** price lookup + catalog status, manufacturers, models, modifications, brands, suppliers, vin, engines, analogs, article-brands, categories, products, engine-search, article-links, article, articles (**16/18**).

**Pending next:** `/api/v1/catalog/engine` → `brand-parts`.
...
- Remaining catalog paths above (`engine`, `brand-parts`)
