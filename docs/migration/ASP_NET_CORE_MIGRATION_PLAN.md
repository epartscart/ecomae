...
## Twenty Fifth Milestone Included Here

- Added `RouteCutoverDecisionMiddleware` to emit `X-EcomAE-Target-Runtime` and `X-EcomAE-PHP-Fallback` headers for the resolved tenant/surface decision.
- The middleware runs after tenant resolution and before endpoint execution, giving proxies, monitors, and smoke tests a stable way to observe cutover decisions without moving production traffic away from PHP.
- Progress increased to 48%; remaining cutover work is proxy feature-flag enforcement, rollout telemetry, per-surface parity approval, and final PHP removal.
...
## Twenty Seventh Milestone Included Here

- Added `/api/v1/catalog/parity` with `ICatalogParityReporter` so catalog API shadow-readiness and remaining PHP fixture/provider gaps are visible next to price parity.
- The endpoint is intentionally lightweight and contract-focused: it keeps PHP authoritative while documenting provider, fixture replay, API-key policy, and tenant scoping gaps.
- Progress increased to 51%; remaining work is implementation depth rather than foundation breadth.


## Twenty Eighth Milestone Included Here
...
- Added `/auth/session/parity` with `ILegacySessionParityReporter` so PHP cookie/header support and remaining session-store/permission gaps are visible separately from API-client parity.
...
## Thirty First Milestone Included Here

- Added `/erp/parity` and `/bos/parity` with focused reporters for Platform ERP and Super BOS/BOC so finance and operations shells have explicit parity diagnostics.
- ERP parity now calls out route aliases, ERP-only tenant classification, finance dashboards, chart-of-accounts, vouchers, invoices, inventory, exports, and tenant-scoped rollback evidence.
- BOS parity now calls out case-insensitive aliases, privileged command-center operations, tenant fleet health, audit evidence, access-denial validation, and rollback controls.
- Progress increased to 57%; remaining work is still deep workflow parity and production data validation, not merely route scaffolding.

## Thirty Second Milestone Included Here

- Added `/storefront/parity` and `/tenant/workspace/parity` so storefront commerce and tenant CP/ERP workspaces now have explicit diagnostic reports.
- Storefront parity documents home/CMS, catalog browsing, cart, checkout, account, SEO metadata, assets, cache headers, and sandbox-payment validation gaps.
- Tenant workspace parity documents live-tenant and ERP-only tenant behavior, menus, user scopes, settings, orders, pricing, finance fixtures, and the remaining move from seeded tenants to production MySQL registry data.
- Progress increased to 60%; remaining work is concentrated in real workflow ports, database-backed parity, staging replay, and final PHP removal.

## Thirty Third Milestone Included Here

- Added `/migration/data-parity` with `IDataParityReporter` to collect the production data sources and replay gates required before replacing PHP responses.
- The report ties catalog/price, API clients, usage logs, tenant registry, and legacy session stores to explicit MySQL/PHP sources and shadow-query evidence.
- Progress increased to 62%; the remaining 38% requires production-like fixture replay, workflow ports, latency validation, feature-flag enforcement, and final PHP removal.
...
## Thirty Fifth Milestone Included Here

- Extended the live smoke runner to support a `/login` probe, username-based super-user credentials via environment variables, and an optional redacted login POST check.
- This lets staging credentials validate controlled auth reachability without hard-coding or printing secrets in scripts or logs.
- Progress remains 68% until production-like workflow replay and data parity can be executed with approved fixtures and a .NET-capable test environment.


## Thirty Sixth Milestone Included Here

- Extended live smoke diagnostics to probe the CloudPanel `/dashboard` path and classify outbound proxy CONNECT tunnel failures separately from app-level failures.
- This makes the provided CloudPanel URL actionable in CI/local runs while preserving the no-secret-logging rule and highlighting when infrastructure access, not application auth, blocks validation.
- Progress remains 68% until the environment can reach staging and run production-like fixture/workflow replay.


## Thirty Seventh Milestone Included Here
