...
3. Presentation parity (heroes/fonts/menus) — in progress (`/marketing/app` hub+home; `/marketing/platform` + `/about` + `/faq`; CP/ERP/BOS/storefront chrome)  
4. Function parity (writes/menus) — in progress (`POST` dry-runs across cart add/qty/check/delete, OMS status/message, ERP cash amend/void + GL manual/reverse + purchase void; `aspNetInteractiveComplete=0` until human dual-sample pass)  
