...
**Surface digest exact-route progress:** **3 / 30** (from `nginx-surface-digests-shadow-example.conf`).

### Surface digest exact-routes still pending

Next: `/cp/groups` → `/cp/modules` → … → ERP/BOS digests (one `location =` at a time). Never broad `/cp|/erp|/bos`.

### Still 100% PHP on public www (blocks Zero-PHP)

- Product chrome: `/`, `/CP/`, `/ERP/`, `/BOS/` (and aliases)
- Remaining CP / ERP / BOS digest exact-routes (27/30) — loopback ASP.NET only until each `location =` is installed
...
- **Surface digests live:** `/cp/dashboard-summary`, `/cp/tenants`, `/cp/users` (3/30).
...
1. Promote remaining CP/ERP/BOS digest exact-routes one `location =` at a time (next `/cp/groups`).
...
- Next install: `ECOMAE_CONFIRM_INSTALL_EXACT_ROUTE_SHADOW=YES bash scripts/cloudpanel_install_exact_route_shadow.sh /cp/groups`
