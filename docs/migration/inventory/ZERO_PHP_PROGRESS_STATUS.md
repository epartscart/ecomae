...
- Batch 1 worker replacement has started at the ASP.NET Core dry-run evidence layer: the worker runner now attaches PHP baseline instructions, ASP.NET dry-run sample text, parity comparison guidance, rollback command text, smoke status, fallback safety, and approvals. This is still not production execution or parity-ready cutover.
