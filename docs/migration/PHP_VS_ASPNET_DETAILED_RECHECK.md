...
**Continuing:** Hybrid UI dual-sample evidence pack (contract stubs under `docs/migration/evidence/hybrid-ui-dual-samples/`; capture/compare scripts; `cutoverAllowed=false`). Live CloudPanel cookie captures remain operator work. Garage #688 and prior slices shipped; profile www preview may still be on an open PR. Batch 6 decommission remains **blocked**. Still never `readyForPhpRemoval`. Same-to-same tenant chrome = PHP until exact-route + dual-sample + human `RELEASE_OWNER_APPROVAL.md`.
