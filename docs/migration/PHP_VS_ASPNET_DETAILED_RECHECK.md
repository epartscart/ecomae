...
**Continuing:** ERP cash & bank Blazor read UI (`/erp/cash-accounts-app`) on www preview only; PHP tab remains authoritative. Invoices #670 / PO #669 shipped. Batch 6 decommission remains **blocked**. Still never `readyForPhpRemoval`. Same-to-same tenant chrome = PHP until exact-route + dual-sample + human `RELEASE_OWNER_APPROVAL.md`.
