...
**Continuing:** CP tenants Blazor read UI (`/cp/tenants-app`) on www preview only; PHP `epc_tenant_control_center` remains authoritative. Audit-log #680 / menus #679 and prior slices shipped. Batch 6 decommission remains **blocked**. Still never `readyForPhpRemoval`. Same-to-same tenant chrome = PHP until exact-route + dual-sample + human `RELEASE_OWNER_APPROVAL.md`.
