...
**Continuing:** BOS audit-log Blazor read UI (`/bos/audit-log-app`) on www preview only; PHP `epc_boc_audit_log` remains authoritative. Menus #679 and prior slices shipped. Batch 6 decommission remains **blocked**. Still never `readyForPhpRemoval`. Same-to-same tenant chrome = PHP until exact-route + dual-sample + human `RELEASE_OWNER_APPROVAL.md`.
