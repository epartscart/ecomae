...
**Code path (after LIVE_PUBLISH of finish-pending branch / main):** ASP.NET fills `DatabaseName=docpart` for epartscart hosts when portal `db_name` is empty — CP login should stop returning `tenant_db_unbound` without portal SQL. Portal bind still recommended so PHP parity + bunches resolver stay aligned.
