...
plus object `payload`. Offline-cache action goldens require object `data` blobs; wave-2 nested
`data.data[]` item floors (`offline-cache-nested-item-field-floor.json`) cover
engines/analogs/article-brands/categories. Price-lookup contract-only requires non-empty `offers[]`.
