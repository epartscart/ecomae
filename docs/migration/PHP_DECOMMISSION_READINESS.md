...
Weighted completion remains **95% / 5% pending**. The residual 5% is PHP runtime decommission only (not “5% of routes left”). Live www wired catalog exact-route shadows are **18/18** complete; CP/ERP/BOS chrome + digests remain PHP. Authenticated staging smoke for price/catalog/surface digests is attached on `main` (PR #612); the remaining final-gate blocker is human `RELEASE_OWNER_APPROVAL.md` after digest shadows + dual samples.
