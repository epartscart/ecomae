<?php
//Серверный скрипт для выставления флага "Кастомная (пользовательская) строка" is_custom для тестовой строки
...
if( !isset($_POST['is_custom']) || !isset($_POST['str_key']) )
{
	$answer = array();
	$answer["status"] = false;
...
if( $db_link->prepare("UPDATE `lang_text_strings` SET `is_custom` = ? WHERE `str_key` = ?;")->execute( array($_POST['is_custom'], $_POST['str_key']) ) )
{
	$answer = array();
	$answer["status"] = true;
	$answer["message"] = "";
	$answer['str_key'] = $_POST['str_key'];
	$answer['is_custom'] = $_POST['is_custom'];
	exit(json_encode($answer));
}
else
{
