<?php
//Серверный скрипт для записи значения колонки same для текстовой строки
...
if( !isset($_POST['same']) || !isset($_POST['str_key']) )
{
	$answer = array();
	$answer["status"] = false;
...
if( $db_link->prepare("UPDATE `lang_text_strings` SET `same` = ? WHERE `str_key` = ?;")->execute( array($_POST['same'], $_POST['str_key']) ) )
{
	$answer = array();
	$answer["status"] = true;
	$answer["message"] = "";
	$answer['str_key'] = $_POST['str_key'];
	$answer['same'] = $_POST['same'];
	exit(json_encode($answer));
}
else
{
