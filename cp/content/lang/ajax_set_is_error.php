<?php
//Серверный скрипт для записи значения колонки is_error для текстовой строки
...
if( !isset($_POST['is_error']) || !isset($_POST['str_key']) )
{
	$answer = array();
	$answer["status"] = false;
...
if( $db_link->prepare("UPDATE `lang_text_strings` SET `is_error` = ? WHERE `str_key` = ?;")->execute( array($_POST['is_error'], $_POST['str_key']) ) )
{
	$answer = array();
	$answer["status"] = true;
	$answer["message"] = "";
	$answer['str_key'] = $_POST['str_key'];
	$answer['is_error'] = $_POST['is_error'];
	exit(json_encode($answer));
}
else
{
