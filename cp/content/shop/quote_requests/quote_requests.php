		...
		<div class="col-lg-12">
			<div class="hpanel">
				...
				<div class="panel-body">
					...
					<script>
					(function () {
						var $modal = jQuery('#epcAltOfferModal');
						function rowByLineId(id) {
							return jQuery('#epc-quote-lines-table tr[data-line-id="' + id + '"]');
						}
						function refreshSummary($row) {
							var on = $row.find('.epc-alt-flag').val() === '1';
							var text = [
								$row.find('.epc-alt-mfr').val(),
								$row.find('.epc-alt-art').val(),
								'× ' + ($row.find('.epc-alt-qty').val() || '1'),
								'@ ' + ($row.find('.epc-alt-price').val() || '')
							].join(' ').replace(/\s+/g, ' ').trim();
							$row.find('.epc-alt-summary-text').text(text);
							$row.find('.epc-alt-summary').toggle(on);
							$row.find('.epc-clear-alt').toggle(on);
							$row.find('.epc-open-alt-modal').text(on ? 'Edit alternative' : 'Amend / alternative');
							$row.find('.epc-quote-main-price').prop('readonly', on);
							if (on) {
								$row.find('.epc-quote-main-price').val($row.find('.epc-alt-price').val());
							}
						}
						jQuery(document).on('click', '.epc-open-alt-modal', function () {
							var $btn = jQuery(this);
							var id = $btn.data('line-id');
							var $row = rowByLineId(id);
							jQuery('#epcAltLineId').val(id);
							jQuery('#epcAltReqLabel').text($btn.data('req-label') || '');
							jQuery('#epcAltReqQty').text($btn.data('req-qty') || '');
							jQuery('#epcAltBrand').val($row.find('.epc-alt-mfr').val() || '');
							jQuery('#epcAltArticle').val($row.find('.epc-alt-art').val() || '');
							jQuery('#epcAltName').val($row.find('.epc-alt-name').val() || '');
							jQuery('#epcAltQty').val($row.find('.epc-alt-qty').val() || $btn.data('req-qty') || 1);
							jQuery('#epcAltPrice').val($row.find('.epc-alt-price').val() || $row.find('.epc-quote-main-price').val() || '');
							$modal.modal('show');
						});
						jQuery('#epcAltApplyBtn').on('click', function () {
							var id = jQuery('#epcAltLineId').val();
							var brand = jQuery.trim(jQuery('#epcAltBrand').val());
							var article = jQuery.trim(jQuery('#epcAltArticle').val());
							var qty = parseInt(jQuery('#epcAltQty').val(), 10) || 0;
							var price = jQuery.trim(jQuery('#epcAltPrice').val()).replace(',', '.');
							if (!brand || !article) {
							...
							var $row = rowByLineId(id);
							$row.find('.epc-alt-flag').val('1');
							$row.find('.epc-alt-mfr').val(brand);
							$row.find('.epc-alt-art').val(article);
							$row.find('.epc-alt-name').val(jQuery.trim(jQuery('#epcAltName').val()));
							$row.find('.epc-alt-qty').val(String(qty));
							$row.find('.epc-alt-price').val(price);
							refreshSummary($row);
							$modal.modal('hide');
						});
						jQuery(document).on('click', '.epc-clear-alt', function () {
							var $row = jQuery(this).closest('tr');
							$row.find('.epc-alt-flag').val('0');
							$row.find('.epc-alt-mfr, .epc-alt-art, .epc-alt-name, .epc-alt-qty, .epc-alt-price').val('');
							refreshSummary($row);
						});
					})();
					</script>
