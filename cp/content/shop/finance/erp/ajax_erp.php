...
try {
	...
	switch ($action) {
		...
		/* ── Global search ──────────────────────────────────────────────── */
		case 'erp_global_search':
			// Read-only: no CSRF token required (matches pattern of other
			// read-only lookup/fetch actions; epc_erp_json() audit hook skips
			// read-only prefixes so 'erp_global_search' is not logged).
			require_once $_SERVER['DOCUMENT_ROOT'] . '/content/shop/finance/epc_erp_global_search.php';
			$gsQ     = trim((string) ($_POST['q'] ?? ''));
			$gsLimit = max(1, min(50, (int) ($_POST['limit'] ?? 20)));
			if (mb_strlen($gsQ, 'UTF-8') < 2) {
				epc_erp_json(true, 'OK', array('results' => array()));
			}
			$gsResults = epc_erp_global_search($db_link, $gsQ, $gsLimit);
			epc_erp_json(true, 'OK', array('results' => $gsResults));
