 ...
 * Ref: Suntech Rate Type Master screenshot.
 */
...
$rateTypes = epc_jewel_rate_type_list($db_link, $companyId);
$divisions = epc_jewel_divisions();

erp_page_header('<i class="fa fa-bar-chart"></i> Rate Type Master', 'Metal rate types with conversion factors.', array(
...
<div class="ef-window">
	...
	<div class="ef-body">
		<table class="ef-grid">
			<thead><tr>
				<th>No.</th><th>Metal</th><th>Rate Type</th><th>Conv Factor</th><th>Conv Factor OZ</th><th>Currency</th><th>Status</th>
			</tr></thead>
			<tbody>
			<?php if (empty($rateTypes)): ?>
				<tr><td colspan="7" style="text-align:center;color:#999">No records — click Seed defaults to populate</td></tr>
			<?php else: $n=1; foreach ($rateTypes as $r): ?>
				<tr class="ef-grid-row" data-id="<?php echo (int)$r['id']; ?>"
					onclick="jwRtSelect(this)" style="cursor:pointer">
					<td><?php echo $n++; ?></td>
					<td><?php echo epc_erp_h($divisions[$r['metal']] ?? $r['metal']); ?></td>
					<td><strong><?php echo epc_erp_h($r['rate_type']); ?></strong></td>
					<td><?php echo number_format((float)$r['conv_factor'], 6); ?></td>
		...
		<div id="jw_rt_form" style="display:none;margin-top:12px;">
			<div class="ef-section">
				<span class="ef-section-title">Rate Type Details</span>
				<form method="POST" action="<?php echo epc_erp_h($erpAjaxUrl); ?>">
					...
					<div class="ef-row">
						...
						<div class="ef-field"><label>Rate Type</label><input name="rate_type" maxlength="10" required placeholder="GMS"></div>
						<div class="ef-field"><label>Currency</label><input name="currency" maxlength="5" value="AED" placeholder="AED"></div>
					</div>
					<div class="ef-row">
						...
						<div class="ef-field"><label>Conv Fac (Gms)</label><input name="conv_factor" type="number" step="0.000001" value="1.000000"></div>
					</div>
					<div class="ef-row">
					...
					<div class="ef-row">
						...
						<div class="ef-field"><label>Default</label>
							<select name="is_default"><option value="0">No</option><option value="1">Yes</option></select>
						</div>
					</div>
