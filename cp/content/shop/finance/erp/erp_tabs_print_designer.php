...
if (is_file($pdBackendFile)) {
	try {
		...
		$_pdBackendErr = $e->getMessage();
	} catch (\Exception $e) {
		$_pdBackendErr = $e->getMessage();
	}
} else {
	$_pdBackendErr = 'File not found: ' . $pdBackendFile;
}

if ($_pdBackendOk && isset($db_link) && $db_link instanceof PDO) {
	try {
		if (function_exists('epc_erp_print_designer_ensure_schema')) {
			epc_erp_print_designer_ensure_schema($db_link);
		}
		if (function_exists('epc_erp_print_designer_seed_defaults')) {
			epc_erp_print_designer_seed_defaults($db_link);
		}
	} catch (\Throwable $e) {
		// Schema init failed — continue
	} catch (\Exception $e) {
		// Schema init failed — continue
	}
}
...
if ($pdAction === 'edit'):
	$tpl = ($pdId > 0 && function_exists('epc_erp_print_template_get')) ? epc_erp_print_template_get($db_link, $pdId) : null;
?>
<div class="ef-window">
	...
	<div class="ef-body">
		<form method="post" action="<?php echo epc_erp_h(isset($erpAjaxUrl) ? $erpAjaxUrl : ''); ?>" class="epc-erp-form">
			...
			<input type="hidden" name="id" value="<?php echo (int)(isset($tpl['id']) ? $tpl['id'] : 0); ?>">
			...
			<div class="row">
						...
						<table class="ef-grid">
							<tr><td style="width:140px"><label>Document type</label></td><td>
								<select name="doc_type" class="form-control input-sm" required>
									...
									<option value="<?php echo epc_erp_h($k); ?>"<?php echo (isset($tpl['doc_type']) && $tpl['doc_type'] === $k) ? ' selected' : ''; ?>><?php echo epc_erp_h($v); ?></option>
									<?php endforeach; ?>
								</select>
							</td></tr>
							<tr><td><label>Template name</label></td><td><input type="text" name="name" class="form-control input-sm" value="<?php echo epc_erp_h(isset($tpl['name']) ? $tpl['name'] : ''); ?>" required></td></tr>
						...
						<table class="ef-grid">
							<tr><td style="width:140px"><label>Page size</label></td><td>
								<select name="page_size" class="form-control input-sm">
									...
									<option<?php echo (isset($tpl['page_size']) ? $tpl['page_size'] : 'A4') === $ps ? ' selected' : ''; ?>><?php echo $ps; ?></option>
									<?php endforeach; ?>
								</select>
							</td></tr>
							<tr><td><label>Orientation</label></td><td>
								<select name="orientation" class="form-control input-sm">
									<option value="portrait"<?php echo (isset($tpl['orientation']) ? $tpl['orientation'] : 'portrait') === 'portrait' ? ' selected' : ''; ?>>Portrait</option>
									<option value="landscape"<?php echo (isset($tpl['orientation']) && $tpl['orientation'] === 'landscape') ? ' selected' : ''; ?>>Landscape</option>
								</select>
							</td></tr>
							<tr><td><label>Margins (mm)</label></td><td>
								T:<input type="number" name="margin_top" class="form-control input-sm" style="width:50px;display:inline" value="<?php echo (int)(isset($tpl['margin_top']) ? $tpl['margin_top'] : 15); ?>">
								B:<input type="number" name="margin_bottom" class="form-control input-sm" style="width:50px;display:inline" value="<?php echo (int)(isset($tpl['margin_bottom']) ? $tpl['margin_bottom'] : 15); ?>">
								L:<input type="number" name="margin_left" class="form-control input-sm" style="width:50px;display:inline" value="<?php echo (int)(isset($tpl['margin_left']) ? $tpl['margin_left'] : 10); ?>">
								R:<input type="number" name="margin_right" class="form-control input-sm" style="width:50px;display:inline" value="<?php echo (int)(isset($tpl['margin_right']) ? $tpl['margin_right'] : 10); ?>">
							</td></tr>
						</table>
					</div>
				</div>
				<div class="col-sm-6">
					<div class="ef-section">
						...
						<table class="ef-grid">
							<tr><td style="width:140px"><label>Font</label></td><td><input type="text" name="font_family" class="form-control input-sm" value="<?php echo epc_erp_h(isset($tpl['font_family']) ? $tpl['font_family'] : 'Arial, sans-serif'); ?>"></td></tr>
							<tr><td><label>Font size (pt)</label></td><td><input type="number" name="font_size" class="form-control input-sm" style="width:70px" value="<?php echo (int)(isset($tpl['font_size']) ? $tpl['font_size'] : 11); ?>"></td></tr>
							<tr><td><label>Primary colour</label></td><td><input type="color" name="primary_color" value="<?php echo epc_erp_h(isset($tpl['primary_color']) ? $tpl['primary_color'] : '#1565c0'); ?>" style="width:50px;height:28px;border:1px solid #ccc;"> <small class="text-muted">Headers, totals</small></td></tr>
							<tr><td><label>Secondary colour</label></td><td><input type="color" name="secondary_color" value="<?php echo epc_erp_h(isset($tpl['secondary_color']) ? $tpl['secondary_color'] : '#4a6a7a'); ?>" style="width:50px;height:28px;border:1px solid #ccc;"> <small class="text-muted">Labels, muted text</small></td></tr>
						</table>
					</div>
					<div class="ef-section" style="margin-top:8px">
						...
						<table class="ef-grid">
							<tr><td style="width:140px"><label>Position</label></td><td>
								<select name="logo_position" class="form-control input-sm">
									...
									<option value="<?php echo $lk; ?>"<?php echo (isset($tpl['logo_position']) ? $tpl['logo_position'] : 'left') === $lk ? ' selected' : ''; ?>><?php echo $lv; ?></option>
									<?php endforeach; ?>
								</select>
							</td></tr>
							<tr><td><label>Max height (px)</label></td><td><input type="number" name="logo_max_height" class="form-control input-sm" style="width:70px" value="<?php echo (int)(isset($tpl['logo_max_height']) ? $tpl['logo_max_height'] : 60); ?>"></td></tr>
						</table>
					</div>
				</div>
			</div>
			...
			<div class="ef-section" style="margin-top:8px">
				...
				<textarea name="header_html" class="form-control" rows="4" placeholder="Header template with merge fields..."><?php echo epc_erp_h(isset($tpl['header_html']) ? $tpl['header_html'] : ''); ?></textarea>
			...
			<div class="ef-section" style="margin-top:8px">
				...
				<textarea name="body_columns" class="form-control" rows="4" placeholder='[{"key":"line_no","label":"#","width":"5%","align":"center"}]'><?php echo epc_erp_h(isset($tpl['body_columns']) ? $tpl['body_columns'] : ''); ?></textarea>
			...
			<div class="row">
				<div class="col-sm-6">
					<div class="ef-section" style="margin-top:8px">
						...
						<textarea name="footer_html" class="form-control" rows="3"><?php echo epc_erp_h(isset($tpl['footer_html']) ? $tpl['footer_html'] : ''); ?></textarea>
					</div>
				</div>
				<div class="col-sm-6">
					<div class="ef-section" style="margin-top:8px">
						<span class="ef-section-title"><i class="fa fa-css3"></i> Custom CSS</span>
						<textarea name="custom_css" class="form-control" rows="3" placeholder="Additional CSS overrides..."><?php echo epc_erp_h(isset($tpl['custom_css']) ? $tpl['custom_css'] : ''); ?></textarea>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-sm-6">
					<div class="ef-section" style="margin-top:8px">
						...
						<textarea name="terms_html" class="form-control" rows="3" placeholder="Terms and conditions text..."><?php echo epc_erp_h(isset($tpl['terms_html']) ? $tpl['terms_html'] : ''); ?></textarea>
					</div>
				</div>
				<div class="col-sm-6">
					<div class="ef-section" style="margin-top:8px">
						...
						<textarea name="bank_details_html" class="form-control" rows="3" placeholder="Bank name, IBAN, SWIFT..."><?php echo epc_erp_h(isset($tpl['bank_details_html']) ? $tpl['bank_details_html'] : ''); ?></textarea>
					</div>
				</div>
			</div>
			...
			<div class="ef-section" style="margin-top:8px">
				...
				<input type="text" name="signature_labels" class="form-control input-sm" value="<?php echo epc_erp_h(isset($tpl['signature_labels']) ? $tpl['signature_labels'] : 'Prepared by,Approved by,Received by'); ?>" placeholder="Comma-separated labels">
				<small class="text-muted">Comma-separated labels for signature boxes</small>
			</div>
...
<?php else: // list view
	$templates = array();
	if ($_pdBackendOk && function_exists('epc_erp_print_templates_list')) {
		try {
...
<div class="ef-window">
	...
	<div class="ef-body">
		<?php if (!empty($_pdBackendErr)): ?>
			<div class="alert alert-warning" style="margin:10px"><i class="fa fa-exclamation-triangle"></i> Print designer backend: <?php echo epc_erp_h($_pdBackendErr); ?></div>
		<?php endif; ?>
		...
		<?php else: ?>
			...
			<div class="ef-section" style="margin-bottom:10px">
				<span class="ef-section-title"><i class="fa fa-file-text-o"></i> <?php echo epc_erp_h(isset($docTypes[$dtype]) ? $docTypes[$dtype] : $dtype); ?></span>
				<table class="ef-grid">
					...
					<?php foreach ($tpls as $t): ?>
						<tr>
							...
							<td><small><?php echo (isset($t['time_updated']) && $t['time_updated'] > 0) ? date('d M Y', (int)$t['time_updated']) : '&mdash;'; ?></small></td>
							<td>
