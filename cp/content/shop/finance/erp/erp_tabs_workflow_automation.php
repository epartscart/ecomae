 ...
 *
 * Wraps the existing epc_workflow_builder to expose workflow automation
 * directly within the ERP interface. Tenants can create approval chains,
...
try {
	...
	$workflows = $st->fetchAll(PDO::FETCH_ASSOC) ?: array();
} catch (Throwable $e) {
	$workflows = array();
}

// Preset workflow templates for common ERP automations
$presets = array(
	array(
		'name' => 'PO Approval — 3-Tier',
		'description' => 'Purchase orders require Manager → Finance → Director approval when amount > threshold.',
		'trigger_type' => 'event',
		'category' => 'Procurement',
		'icon' => 'fa-shopping-cart',
	),
	array(
		'name' => 'Invoice auto-send on validation',
		'description' => 'Automatically email the PDF invoice to the customer when invoice status changes to Validated.',
		'trigger_type' => 'event',
		'category' => 'Sales',
		'icon' => 'fa-paper-plane',
	),
	array(
		...
		'description' => 'Send notification when inventory item quantity drops below reorder level.',
		'trigger_type' => 'event',
		'category' => 'Inventory',
		'icon' => 'fa-exclamation-triangle',
	),
	array(
		'name' => 'Monthly VAT preparation reminder',
		'description' => 'Send reminder 7 days before VAT return due date to prepare filing.',
		'trigger_type' => 'schedule',
		'category' => 'Compliance',
		'icon' => 'fa-calendar-check-o',
	),
	array(
		'name' => 'Customer payment overdue escalation',
		'description' => 'Escalate to finance manager when customer payment is 30+ days overdue.',
		'trigger_type' => 'schedule',
		'category' => 'Finance',
		'icon' => 'fa-clock-o',
	),
	array(
		...
		'description' => 'Create onboarding tasks when new employee is added to HR module.',
		'trigger_type' => 'event',
		'category' => 'HR',
		'icon' => 'fa-user-plus',
	),
	array(
		...
		'description' => 'Send daily summary of sales, returns, and top items to management.',
		'trigger_type' => 'schedule',
		'category' => 'Sales',
		'icon' => 'fa-bar-chart',
	),
	array(
		'name' => 'AML — large cash transaction alert',
		'description' => 'Flag cash transactions above AED 55,000 for AML review (jewellery DPMS).',
		'trigger_type' => 'event',
		'category' => 'Compliance',
		'icon' => 'fa-shield',
	),
...
if ($wfAction === 'edit'):
	...
	if ($wfId > 0) {
		...
		if ($wf) {
			...
			$steps = $sSt->fetchAll(PDO::FETCH_ASSOC) ?: array();
		}
	}
?>
<div class="ef-window">
	...
	<div class="ef-body">
		<form method="post" action="<?php echo epc_erp_h($erpAjaxUrl); ?>" class="epc-erp-form">
			...
			<input type="hidden" name="id" value="<?php echo (int)($wf['id'] ?? 0); ?>">

			<div class="ef-section">
				<span class="ef-section-title"><i class="fa fa-info-circle"></i> Workflow details</span>
				<table class="ef-grid">
					<tr><td style="width:140px"><label>Name</label></td><td><input type="text" name="name" class="form-control input-sm" value="<?php echo epc_erp_h($wf['name'] ?? ''); ?>" required></td></tr>
					<tr><td><label>Description</label></td><td><textarea name="description" class="form-control input-sm" rows="2"><?php echo epc_erp_h($wf['description'] ?? ''); ?></textarea></td></tr>
					<tr><td><label>Trigger type</label></td><td>
						<select name="trigger_type" class="form-control input-sm">
							<option value="event"<?php echo ($wf['trigger_type'] ?? '') === 'event' ? ' selected' : ''; ?>>Event-driven (on record change)</option>
							<option value="schedule"<?php echo ($wf['trigger_type'] ?? '') === 'schedule' ? ' selected' : ''; ?>>Scheduled (cron)</option>
							<option value="manual"<?php echo ($wf['trigger_type'] ?? 'manual') === 'manual' ? ' selected' : ''; ?>>Manual (button trigger)</option>
							<option value="webhook"<?php echo ($wf['trigger_type'] ?? '') === 'webhook' ? ' selected' : ''; ?>>Webhook (external API call)</option>
						</select>
					</td></tr>
			...
			<div class="ef-section" style="margin-top:8px">
				...
				<?php else: ?>
				<div style="padding:10px;color:#999;font-size:12px;"><i class="fa fa-info-circle"></i> Steps will be configurable after saving the workflow. Use the visual builder in BOS for drag-and-drop step ordering.</div>
				<?php endif; ?>
			</div>
		...
		<?php else: ?>
			<table class="ef-grid">
				...
				<?php foreach ($workflows as $w): ?>
					<tr>
						<td><strong><?php echo epc_erp_h($w['name']); ?></strong><br><small class="text-muted"><?php echo epc_erp_h(substr($w['description'] ?? '', 0, 60)); ?></small></td>
						<td><span class="label label-info"><?php echo epc_erp_h($w['trigger_type']); ?></span></td>
						<td><?php echo (int)$w['run_count']; ?></td>
						<td><small><?php echo $w['last_run_at'] ? epc_erp_h($w['last_run_at']) : '—'; ?></small></td>
