<?php
/**
 * Jewellery ERP — Sales Fixing.
 * Ref: Suntech Sales Fixing screenshot.
 */
...
$fixings = epc_jewel_fixing_list($db_link, $companyId, 'SALE');

erp_page_header('<i class="fa fa-lock"></i> Sales Fixing', 'Fix metal rates against unfixed sales vouchers.', array(
	array('label' => 'ERP', 'url' => epc_erp_tab_url($erpUrl, 'dashboard', $date_from_str, $date_to_str)),
	array('label' => 'Jewellery', 'url' => epc_erp_tab_url($erpUrl, 'jewellery', $date_from_str, $date_to_str)),
	array('label' => 'Sales fixing'),
));
?>
<div class="ef-window">
	<div class="ef-title">Sales Fixing</div>
	<div class="ef-toolbar">
		<button class="btn btn-default btn-xs" onclick="document.getElementById('jw_sf_form').style.display='block'"><i class="fa fa-plus"></i> New Fixing</button>
	...
	<div class="ef-body">
		...
		<div id="jw_sf_form" style="display:none;margin-top:12px;">
			<form method="POST" action="<?php echo epc_erp_h($erpAjaxUrl); ?>">
			<input type="hidden" name="csrf_guard_key" value="<?php echo epc_erp_h($csrfLocal); ?>">
			<input type="hidden" name="action" value="jw_sales_fixing_save">
			...
			<div class="ef-section">
				...
				<div class="ef-row">
					<div class="ef-field"><label>Ref Voc Type</label><input name="ref_voc_type" maxlength="5" value="MSL"></div>
					<div class="ef-field"><label>Ref Voc No</label><input name="ref_voc_no" maxlength="20" required></div>
			...
			<div class="ef-actions">
				...
				<button type="button" class="btn btn-default btn-sm" onclick="document.getElementById('jw_sf_form').style.display='none'">Cancel</button>
			</div>
			</form>
		</div>
	</div>
