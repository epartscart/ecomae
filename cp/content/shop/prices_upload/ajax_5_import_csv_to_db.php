...
$epc_total_skipped = 0;
$epc_import_issues = array();
$epc_line_no = (int)$strings_to_left;
$epc_archived_file = '';
$epc_archived_name = '';
$epc_archived_size = 0;
...
while (false !== ($obj = readdir($dh)))
{
	...
	else
	{
		...
		$epc_header_row = null;
		if ($strings_to_left > 0) {
			$epc_header_row = fgetcsv($file, 0, $csv_delimiter);
			for ($i = 1; $i < $strings_to_left; $i++) {
				fgetcsv($file, 0, $csv_delimiter);
			}
		}
		$epc_column_labels = epc_price_build_source_column_labels(
			is_array($epc_header_row) ? $epc_header_row : null,
			$operational_cols
		);
		
		//На время загрузки отключаем индексы
		epc_ensure_prices_data_exist_column_int($db_link);
		...
		{	
			$epc_line_no++;
			if (count($current_record) > count($epc_column_labels)) {
				$epc_column_labels = epc_price_build_source_column_labels(
					is_array($epc_header_row) ? $epc_header_row : null,
					$operational_cols,
					count($current_record)
				);
			}
			if( !is_array($current_record) || (count($current_record) === 1 && trim((string)$current_record[0]) === '') )
			{
				$epc_import_issues[] = epc_price_history_issue($epc_line_no, 'skipped', 'empty_row', array(
					'_csv_row' => is_array($current_record) ? $current_record : array(),
					'_column_labels' => $epc_column_labels,
				));
				$epc_total_skipped++;
				continue;
			}
			...
			{
				$exist = epc_parse_stock_quantity($current_record[$operational_cols["exist"]-1] ?? '');
			}
			$price = 0;
			...
			if( $article === "" )
			{
				$epc_import_issues[] = epc_price_history_issue($epc_line_no, 'skipped', 'empty_article', array(
					'_csv_row' => $current_record,
					'_column_labels' => $epc_column_labels,
					...
					'exist' => (string)$exist,
					'price' => (string)$price,
					'details' => 'Article column value: "' . $article_show . '"',
				));
				$epc_total_skipped++;
				continue;
			}
			if( $price <= 0 )
			{
				$epc_import_issues[] = epc_price_history_issue($epc_line_no, 'skipped', 'invalid_price', array(
					'_csv_row' => $current_record,
					'_column_labels' => $epc_column_labels,
					'manufacturer' => $manufacturer,
					'article' => $article,
					...
					'exist' => (string)$exist,
					'price' => (string)$price,
					'details' => 'Price column value: "' . (string)$price . '"',
				));
				$epc_total_skipped++;
				continue;
			}
...
$epc_history_status = 'failed';
if( $epc_total_imported > 0 )
{
	$epc_history_status = ($epc_total_skipped > 0 || count($epc_import_issues) > 0) ? 'partial' : 'ok';
}
$epc_history_id = epc_price_history_save($db_link, array(
	...
	'rows_skipped' => $epc_total_skipped,
	'rows_in_db' => $epc_rows_in_db,
	'status' => $epc_history_status,
...
$answer["rows_skipped"] = $epc_total_skipped;
$answer["history_id"] = $epc_history_id;
exit(json_encode($answer));
?>