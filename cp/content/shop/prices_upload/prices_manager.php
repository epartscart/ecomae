		...
		<div class="hpanel">
			...
			<div class="panel-body">
				...
				<div class="epc-cp-table-filter-bar" style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin:0 0 12px;">
					<label for="epc_prices_filter" style="margin:0;font-weight:700;font-size:12px;color:#64748b;">Find price list</label>
					<input type="search" id="epc_prices_filter" class="form-control input-sm" style="max-width:340px;" placeholder="Name, ID, warehouse…" autocomplete="off" />
					<span class="text-muted small">Showing <strong id="epc_prices_filter_count">—</strong> price lists</span>
				</div>
					...
					<!--<table cellpadding="1" cellspacing="1" class="table table-condensed table-striped">-->
						...
						*/
						...
						foreach( $epc_prices_list_rows as $element_record )
						{
							...
							$epc_price_wh = isset($shop_storages_link_prices[$element_record["id"]])
								? strip_tags((string) $shop_storages_link_prices[$element_record["id"]])
								: '';
							$epc_price_filter = strtolower(trim(
								(string) $element_record['id'] . ' ' .
								(string) $element_record['name'] . ' ' .
								$epc_price_wh . ' ' .
								(string) ($element_record['records_count'] ?? '')
							));
							?>
						
						
							<tr data-epc-filter="<?php echo htmlspecialchars($epc_price_filter, ENT_QUOTES, 'UTF-8'); ?>" onclick="get_price_preview_<?php echo $element_record["id"]; ?>();">
		...
		jQuery( window ).load(function() {
			...
			if (typeof window.epcCpBindTableFilter === 'function') {
				window.epcCpBindTableFilter({
					inputId: 'epc_prices_filter',
					tableId: 'prices_table',
					countId: 'epc_prices_filter_count'
				});
			}
		});
	</script>
