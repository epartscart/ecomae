<?php
/**
 * BOC — Vendor & Sourcing Control (route). Fleet-wide multi-vendor control:
 * suppliers, open RFQs and purchase spend across every tenant, in the BOC shell.
 */
...
if (!function_exists('epc_portal_is_super_cp_host') || !epc_portal_is_super_cp_host()) {
    echo '<div class="alert alert-warning">Vendor control is available on <strong>BOC</strong> (Super CP) only.</div>';
    return;
}
...
epc_boc_console_open(array('active' => 'vendor_control', 'title' => 'Vendor & Sourcing', 'base' => $base, 'operator' => $operatorName, 'env' => 'Production', 'nav' => $nav, 'scope' => 'All units · Fleet'));
epc_boc_render_vendor_control($db_link, $base);
