<?php
/**
 * Register Procurement CP module.
 * Run: https://www.epartscart.com/epc-procurement-setup.php?token=epartscart-deploy-2026
 */
...
require_once __DIR__ . '/content/shop/procurement/epc_procurement_schema.php';
...
function epc_proc_lang($pdo, $key, $en, $ru)
{
...
epc_proc_lang($pdo, 'epc_procurement_cp', 'Procurement & suppliers', 'Закупки и поставщики');
epc_proc_lang($pdo, 'epc_cp_group_procurement', 'Procurement', 'Закупки');

epc_procurement_ensure_schema($pdo);

$procFolderId = epc_cp_breadcrumb_ensure_folder_content(
	$pdo,
	'shop',
	'shop/procurement',
	'procurement_folder',
...
function epc_proc_register_content(PDO $pdo, $parentUrl, $url, $alias, $valueKey, $phpPath, $title, $order = 88)
{
...
$contentId = epc_proc_register_content(
	$pdo,
	'shop/procurement',
	'shop/procurement/procurement',
	'procurement',
	'epc_procurement_cp',
	'/<backend_dir>/content/shop/procurement/procurement_main_page.php',
	'Procurement & suppliers',
	88
);

$menu = epc_cp_procurement_menu_apply($pdo);
$base = rtrim($cfg->domain_path, '/');

echo json_encode(array(
	'status' => true,
	'message' => 'Procurement module registered',
	'content_id' => $contentId,
	'procurement_folder_id' => $procFolderId,
	'menu_group_id' => $menu['procurement_group'],
	'control_items_id' => $menu['items']['procurement_hub'],
	'urls' => array(
		'cp_hub' => $base . '/' . $cfg->backend_dir . '/shop/procurement/procurement',
		'setup_lockdown_safe' => $base . '/procurement-setup.php?token=REDACTED',
	),
