<?php
/**
 * Marketing Broadcast — schema, CP routes, menu on platform + live tenants.
 *
 * Dry-run:  https://www.ecomae.com/epc-marketing-broadcast-setup-all.php?token=epartscart-deploy-2026
 * Apply:    …&apply=1
...
$pages = array(
	array('slug' => 'epc_marketing_broadcast', 'key' => 'epc_marketing_broadcast_cp', 'en' => 'Marketing broadcast', 'ru' => 'Рассылка — маркетинг', 'order' => 13),
);
...
function epc_mb_setup_connect(array $cred, DP_Config $cfg): ?PDO
{
...
echo "=== ECOM AE Marketing Broadcast Setup All ===\n";
...
foreach ($unique as $target) {
	...
	$pdo = epc_mb_setup_connect($target['cred'], $cfg);
	if (!$pdo instanceof PDO) {
		echo "  CONNECT FAILED\n\n";
		continue;
	}
	epc_mb_ensure_schema($pdo);
	echo "  schema: epc_marketing_broadcast_campaigns + log OK\n";
	...
	if ($apply) {
		$menu = epc_cp_marketing_broadcast_menu_apply($pdo);
		echo '  menu portal_group=' . (int) ($menu['portal_group'] ?? 0) . ' broadcast_item=' . (int) ($menu['marketing_broadcast_item'] ?? 0) . "\n";
	}
	echo "\n";
}
...
if ($apply) {
	...
	echo "  https://www.ecomae.com/cp/control/portal/epc_marketing_broadcast\n";
	echo "  Guide: https://www.ecomae.com/cp/control/portal/epc_marketing_broadcast?tab=guide\n";
	echo "Verify Tenant CP:\n";
	echo "  https://www.epartscart.com/cp/control/portal/epc_marketing_broadcast\n";
	echo "  Guide: https://www.epartscart.com/cp/control/portal/epc_marketing_broadcast?tab=guide\n";
	echo "Probe: https://www.ecomae.com/epc-marketing-broadcast-verify.php?token=epartscart-deploy-2026\n";
}
