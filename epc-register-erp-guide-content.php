<?php
/**
 * Register CP URL shop/finance/erp/guide (repair helper).
...
$parentUrl = 'shop/finance/erp';
$guideUrl = 'shop/finance/erp/guide';
$phpPath = '/<backend_dir>/content/shop/finance/erp/erp_guide_page.php';
$now = time();
...
if (!$parentRow) {
	exit(json_encode(array('status' => false, 'message' => 'Parent shop/finance/erp not found — run epc-erp-cp-setup.php first')));
}
...
if ($contentId > 0) {
	$db->prepare(
		'UPDATE `content` SET `published_flag` = 1, `content_type` = \'php\', `is_frontend` = 0,
		 `content` = ?, `title_tag` = ?, `parent` = ?, `level` = ?, `alias` = \'guide\', `url` = ?
		 WHERE `id` = ?'
	)->execute(array($phpPath, 'ERP guide — eParts Cart', (int)$parentRow['id'], (int)$parentRow['level'] + 1, $guideUrl, $contentId));
} else {
	$db->prepare(
		'INSERT INTO `content`
		(`count`, `url`, `level`, `alias`, `value`, `parent`, `description`, `is_frontend`, `content_type`, `content`,
		 ...
		 VALUES (0, ?, ?, \'guide\', \'epc_erp_guide_cp\', ?, ?, 0, \'php\', ?, ?, \'0\', \'0\', \'0\', 0, \'[]\', \'\', \'\', 0, 1, 0, ?, ?, 1)'
	)->execute(array(
		$guideUrl,
