...
public sealed class WaveBErpAjaxRegistryDryRunTests
{
    ...
    [Fact]
    public void EditLockHeartbeatValidated()
    {
        var r = new ErpEditLockHeartbeatDryRun().Evaluate(new("so:1"));
        Assert.Equal("dry-run-validated", r.Status);
        Assert.Equal(0, r.Writes);
        Assert.False(r.CutoverAllowed);
        Assert.Equal(
            "confirm_writes_refused",
            new ErpEditLockHeartbeatDryRun().Evaluate(new("so:1", true)).ValidationCode);
    }
