...
public sealed class BosSoc2EvidenceWriteTests
{
    [Fact]
    public void Route_exposes_add_evidence()
    {
        Assert.Equal("/bos/soc2/add-evidence", EcomAeRoutes.BosSoc2AddEvidence);
    }

    [Fact]
    public void Page_posts_native_add_evidence()
    {
        ...
        Assert.Contains("action=\"/bos/soc2/add-evidence\"", razor, StringComparison.Ordinal);
        Assert.Contains("name=\"confirmWrites\"", razor, StringComparison.Ordinal);
        Assert.Contains("value=\"true\"", razor, StringComparison.Ordinal);
        Assert.Contains("name=\"control_id\"", razor, StringComparison.Ordinal);
        Assert.Contains("name=\"evidence_type\"", razor, StringComparison.Ordinal);
    ...
    [Fact]
    public void Catalog_marks_add_evidence_live_gated()
    {
        var write = SurfacePayloadContractCatalog.Functions.First(item =>
            item.AspNetRouteOrCapability == "/bos/soc2/add-evidence");
        Assert.Equal("write-live-gated", write.Status);
        Assert.Contains("add_evidence", write.Notes, StringComparison.Ordinal);
        Assert.Contains("epc_soc2_evidence", write.Notes, StringComparison.Ordinal);
    ...
    [Fact]
    public void Program_and_module_add_evidence_write()
    {
        var module = File.ReadAllText(Path.Combine(FindRepoRoot(), "aspnet/src/EcomAE.Platform/Modules/BosModule.cs"));
        Assert.Contains("BosSoc2AddEvidence", module, StringComparison.Ordinal);
        Assert.Contains("AddEvidenceAsync", module, StringComparison.Ordinal);
        ...
        Assert.Contains("INSERT INTO `epc_soc2_evidence`", service, StringComparison.Ordinal);
