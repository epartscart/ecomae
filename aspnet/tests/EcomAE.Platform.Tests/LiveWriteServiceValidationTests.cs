...
public sealed class LiveWriteServiceValidationTests
{
    ...
    public async Task Quote_accept_add_and_customer_writes_reject_before_open()
    {
        ...
        var bosAllReadDb = await new BosNotificationWriteService(new UnconfiguredConnections())
            .MarkAllReadAsync("__platform__", 0);
        Assert.False(bosAllReadDb.Succeeded);
        Assert.Equal("db", bosAllReadDb.Code);
        ...
        var bosPrefsDb = await new BosNotificationWriteService(new UnconfiguredConnections())
            .SavePrefsAsync("__platform__", 0, "*", "1", "1", "0", "daily");
        ...
        var bosReleaseDb = await new BosCreditWriteService(new UnconfiguredConnections())
            .ReleaseAsync("acme", 9);
        ...
        Assert.Equal("db", bosApiRevokeDb.Code);

        var bosWebhookDeleteDb = await new BosWebhookWriteService(new UnconfiguredConnections())
            .DeleteAsync(9);
        Assert.False(bosWebhookDeleteDb.Succeeded);
        Assert.Equal("db", bosWebhookDeleteDb.Code);

        var bosDlqResolveDb = await new BosWebhookWriteService(new UnconfiguredConnections())
            .ResolveDlqAsync(9);
        ...
        var bosPoApproveDb = await new BosPoWriteService(new UnconfiguredConnections())
            .ApproveAsync(9, 1, 2, "ok");
        Assert.False(bosPoApproveDb.Succeeded);
        Assert.Equal("db", bosPoApproveDb.Code);
        ...
        Assert.Equal("db", bosBillCancelDb.Code);

        var bosBillPayDb = await new BosBillingWriteService(new UnconfiguredConnections())
            .PayAsync(9, "card");
        ...
        var bosBillSubscribeDb = await new BosBillingWriteService(new UnconfiguredConnections())
            .SubscribeAsync("acme", 9);
        Assert.False(bosBillSubscribeDb.Succeeded);
        Assert.Equal("db", bosBillSubscribeDb.Code);
        ...
        Assert.Equal("db", bosDealerDb.Code);

        var bosDunningDb = await new BosDunningWriteService(new UnconfiguredConnections())
            .UpdateStatusAsync(9, "open", "");
        Assert.False(bosDunningDb.Succeeded);
        Assert.Equal("db", bosDunningDb.Code);

        var bosDunningPayDb = await new BosDunningWriteService(new UnconfiguredConnections())
            .RecordPaymentAsync(9, 10m);
        Assert.False(bosDunningPayDb.Succeeded);
        Assert.Equal("db", bosDunningPayDb.Code);

        var bosDunningProfileDb = await new BosDunningWriteService(new UnconfiguredConnections())
            .CreateProfileAsync("acme", "Default", null);
        ...
        Assert.Equal("db", bosSsoToggleDb.Code);

        var bosSsoDeleteDb = await new BosSsoWriteService(new UnconfiguredConnections())
            .DeleteAsync(9);
        Assert.False(bosSsoDeleteDb.Succeeded);
        Assert.Equal("db", bosSsoDeleteDb.Code);

        var bosSandboxPromoteDb = await new BosSandboxWriteService(new UnconfiguredConnections())
            .PromoteAsync(9);
        ...
        var bosSandboxApplyChangeDb = await new BosSandboxWriteService(new UnconfiguredConnections())
            .ApplyChangeAsync(9, "brand_name", "old", "new", "modify");
        ...
        var bosMarketplaceReviewDb = await new BosMarketplaceWriteService(new UnconfiguredConnections())
            .AddReviewAsync(9, "acme", "{\"rating\":5}");
        Assert.False(bosMarketplaceReviewDb.Succeeded);
        Assert.Equal("db", bosMarketplaceReviewDb.Code);

        var bosMarketplaceInstallDb = await new BosMarketplaceWriteService(new UnconfiguredConnections())
            .InstallAsync(9, "acme", 0);
        ...
        var bosPromoCreateDb = await new BosPromoWriteService(new UnconfiguredConnections())
            .CreateAsync("acme", null);
        Assert.False(bosPromoCreateDb.Succeeded);
        Assert.Equal("db", bosPromoCreateDb.Code);
        ...
        Assert.Equal("db", bosEntityEliminateDb.Code);

        var bosPayrollApproveDb = await new BosPayrollWriteService(new UnconfiguredConnections())
            .ApproveRunAsync(9, 1);
        ...
        var bosVaultUploadDb = await new BosVaultWriteService(new UnconfiguredConnections())
            .UploadAsync("acme", "{\"filename\":\"a.bin\"}");
        Assert.False(bosVaultUploadDb.Succeeded);
        Assert.Equal("db", bosVaultUploadDb.Code);

        var bosImportCreateDb = await new BosImportWriteService(new UnconfiguredConnections())
            .CreateJobAsync("acme", null);
        ...
        var bosTokenSaveDb = await new BosDesignTokenWriteService(new UnconfiguredConnections())
            .SaveTokenAsync("acme", "brand_primary", "#111");
        Assert.False(bosTokenSaveDb.Succeeded);
        Assert.Equal("db", bosTokenSaveDb.Code);

        var bosTenantConfigDb = await new BosTenantConfigWriteService(new UnconfiguredConnections())
            .SetAsync("acme", "branding", "company_name", "Acme", 0);
        ...
        Assert.Equal("db", bosMfaSaveDb.Code);
