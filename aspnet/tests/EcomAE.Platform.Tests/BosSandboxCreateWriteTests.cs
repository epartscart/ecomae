...
public sealed class BosSandboxCreateWriteTests
{
    [Fact]
    public void Route_exposes_create()
    {
        Assert.Equal("/bos/sandbox/create", EcomAeRoutes.BosSandboxCreate);
    }

    [Fact]
    public void Php_site_key_and_config_match_php()
    {
        Assert.Equal("acme1", BosSandboxWriteService.PhpBosSiteKey("Acme-1!"));
        Assert.Equal("acme_1", BosSandboxWriteService.PhpBosSiteKey("Acme_1"));
        Assert.Equal("", BosSandboxWriteService.PhpBosSiteKey("!!!"));
        Assert.Equal("", BosSandboxWriteService.PhpBosSiteKey(null));

        Assert.Equal("[]", BosSandboxWriteService.SerializeConfigData(null));
        Assert.Equal("[]", BosSandboxWriteService.SerializeConfigData(""));
        Assert.Equal("[]", BosSandboxWriteService.SerializeConfigData("{}"));
        Assert.Equal("[]", BosSandboxWriteService.SerializeConfigData("not-json"));
        Assert.Equal("[]", BosSandboxWriteService.SerializeConfigData("[]"));

        var json = BosSandboxWriteService.SerializeConfigData("{\"brand\":\"acme\"}");
        Assert.Contains("brand", json, StringComparison.Ordinal);
    ...
    {
        var written = await new BosSandboxWriteService(new UnconfiguredConnections())
            .CreateAsync("!!!", "Sandbox", null, 0);
    ...
    public void Page_posts_native_create()
    {
        ...
        Assert.Contains("action=\"/bos/sandbox/create\"", razor, StringComparison.Ordinal);
        ...
        Assert.Contains("name=\"config_data\"", razor, StringComparison.Ordinal);
    ...
    public void Catalog_marks_create_live_gated()
    {
        var write = SurfacePayloadContractCatalog.Functions.First(item =>
            item.AspNetRouteOrCapability == "/bos/sandbox/create");
        Assert.Equal("write-live-gated", write.Status);
        Assert.Contains("epc_sandbox_create", write.Notes, StringComparison.Ordinal);
        Assert.Contains("epc_config_snapshots", write.Notes, StringComparison.Ordinal);
    ...
    public void Program_and_module_create_write()
    {
        ...
        Assert.Contains("BosSandboxCreate", module, StringComparison.Ordinal);
        ...
        Assert.Contains("epc_sandbox_create", service, StringComparison.Ordinal);
        Assert.Contains("INSERT INTO `epc_config_snapshots`", service, StringComparison.Ordinal);
