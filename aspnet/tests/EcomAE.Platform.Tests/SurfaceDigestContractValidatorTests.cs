...
public sealed class SurfaceDigestContractValidatorTests
{
    ...
    public async Task MigrationDigestsSatisfyFieldContractsForSummaryRoutes()
    {
        ...
        var payloads = new Dictionary<string, object>
        {
            ...
            ["/erp/inventory-stock"] = new
            {
                ...
                lowStock = (await reporter.BuildErpInventoryStockDigestAsync(10)).LowStock,
                count = 0,
                source = "migration",
                message = "x",
                session,
    ...
    public async Task MigrationListDigestsSatisfyItemFieldContractsWhenEmpty()
    {
        ...
        var listPayloads = new Dictionary<string, object>
        {
            ...
            ["/cp/crm-activities"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpCrmActivitiesDigestAsync(10)).Summary, activities = (await reporter.BuildCpCrmActivitiesDigestAsync(10)).Activities, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/auth-mfa"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpAuthMfaDigestAsync(10)).Summary, secrets = (await reporter.BuildCpAuthMfaDigestAsync(10)).Secrets, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/electronic-reporting"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpElectronicReportingDigestAsync(10)).Summary, formats = (await reporter.BuildCpElectronicReportingDigestAsync(10)).Formats, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/collections-dunning"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpCollectionsDunningDigestAsync(10)).Summary, queue = (await reporter.BuildCpCollectionsDunningDigestAsync(10)).Queue, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/marketplace-channels"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpMarketplaceChannelsDigestAsync(10)).Summary, channels = (await reporter.BuildCpMarketplaceChannelsDigestAsync(10)).Channels, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/demand-intelligence"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpDemandIntelligenceDigestAsync(10)).Summary, countries = (await reporter.BuildCpDemandIntelligenceDigestAsync(10)).Countries, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/credit-limits"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpCreditLimitsDigestAsync(10)).Summary, limits = (await reporter.BuildCpCreditLimitsDigestAsync(10)).Limits, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/insurance-compliance"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpInsuranceComplianceDigestAsync(10)).Summary, policies = (await reporter.BuildCpInsuranceComplianceDigestAsync(10)).Policies, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/audit-trail"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpAuditTrailDigestAsync(10)).Summary, entries = (await reporter.BuildCpAuditTrailDigestAsync(10)).Entries, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/doc-expiry"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpDocExpiryDigestAsync(10)).Summary, documents = (await reporter.BuildCpDocExpiryDigestAsync(10)).Documents, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/tenant-config"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpTenantConfigDigestAsync(10)).Summary, entries = (await reporter.BuildCpTenantConfigDigestAsync(10)).Entries, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/jewellery-stock-verification"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpJewelleryStockVerificationDigestAsync(10)).Summary, verifications = (await reporter.BuildCpJewelleryStockVerificationDigestAsync(10)).Verifications, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/tax-external-reporting"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpTaxExternalReportingDigestAsync(10)).Summary, rules = (await reporter.BuildCpTaxExternalReportingDigestAsync(10)).Rules, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/po-approvals"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpPoApprovalsDigestAsync(10)).Summary, requests = (await reporter.BuildCpPoApprovalsDigestAsync(10)).Requests, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/finance-close"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpFinanceCloseDigestAsync(10)).Summary, batches = (await reporter.BuildCpFinanceCloseDigestAsync(10)).Batches, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/jewellery-fixing"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpJewelleryFixingDigestAsync(10)).Summary, fixings = (await reporter.BuildCpJewelleryFixingDigestAsync(10)).Fixings, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/web-tracker"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpWebTrackerDigestAsync(10)).Summary, sessions = (await reporter.BuildCpWebTrackerDigestAsync(10)).Sessions, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/abandoned-carts"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpAbandonedCartsDigestAsync(10)).Summary, carts = (await reporter.BuildCpAbandonedCartsDigestAsync(10)).Carts, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/quote-requests"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpQuoteRequestsDigestAsync(10)).Summary, quotes = (await reporter.BuildCpQuoteRequestsDigestAsync(10)).Quotes, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/platform-communication"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpPlatformCommunicationDigestAsync(10)).Summary, tasks = (await reporter.BuildCpPlatformCommunicationDigestAsync(10)).Tasks, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/info-blocks"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpInfoBlocksDigestAsync(10)).Summary, blocks = (await reporter.BuildCpInfoBlocksDigestAsync(10)).Blocks, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/free-tools"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpFreeToolsDigestAsync(10)).Summary, accounts = (await reporter.BuildCpFreeToolsDigestAsync(10)).Accounts, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/config-sandbox"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpConfigSandboxDigestAsync(10)).Summary, snapshots = (await reporter.BuildCpConfigSandboxDigestAsync(10)).Snapshots, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/marketplace-apps"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpMarketplaceAppsDigestAsync(10)).Summary, apps = (await reporter.BuildCpMarketplaceAppsDigestAsync(10)).Apps, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/notifications"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpNotificationsDigestAsync(10)).Summary, notifications = (await reporter.BuildCpNotificationsDigestAsync(10)).Notifications, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/portal-settings"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpPortalSettingsDigestAsync(10)).Summary, sites = (await reporter.BuildCpPortalSettingsDigestAsync(10)).Sites, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ...
            ["/cp/geo-regions"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpGeoRegionsDigestAsync(10)).Summary, nodes = (await reporter.BuildCpGeoRegionsDigestAsync(10)).Nodes, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/product-filters"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpProductFiltersDigestAsync(10)).Summary, filters = (await reporter.BuildCpProductFiltersDigestAsync(10)).Filters, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/search-tabs"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpSearchTabsDigestAsync(10)).Summary, tabs = (await reporter.BuildCpSearchTabsDigestAsync(10)).Tabs, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/system-requests"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpSystemRequestsDigestAsync(10)).Summary, requests = (await reporter.BuildCpSystemRequestsDigestAsync(10)).Requests, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/additional-texts"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpAdditionalTextsDigestAsync(10)).Summary, texts = (await reporter.BuildCpAdditionalTextsDigestAsync(10)).Texts, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/slider-banners"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpSliderBannersDigestAsync(10)).Summary, images = (await reporter.BuildCpSliderBannersDigestAsync(10)).Images, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/structure-dumps"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpStructureDumpsDigestAsync(10)).Summary, dumps = (await reporter.BuildCpStructureDumpsDigestAsync(10)).Dumps, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/communications-test"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpCommunicationsTestDigestAsync(10)).Summary, channels = (await reporter.BuildCpCommunicationsTestDigestAsync(10)).Channels, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/languages"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpLanguagesDigestAsync(10)).Summary, languages = (await reporter.BuildCpLanguagesDigestAsync(10)).Languages, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/plugins-manager"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpPluginsManagerDigestAsync(10)).Summary, plugins = (await reporter.BuildCpPluginsManagerDigestAsync(10)).Plugins, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/templates-manager"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpTemplatesManagerDigestAsync(10)).Summary, templates = (await reporter.BuildCpTemplatesManagerDigestAsync(10)).Templates, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/design-tokens"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpDesignTokensDigestAsync(10)).Summary, tokens = (await reporter.BuildCpDesignTokensDigestAsync(10)).Tokens, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ...
            ["/cp/failover-status"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpFailoverStatusDigestAsync(10)).Summary, signals = (await reporter.BuildCpFailoverStatusDigestAsync(10)).Signals, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/ops-guides"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpOpsGuidesDigestAsync(10)).Summary, items = (await reporter.BuildCpOpsGuidesDigestAsync(10)).Items, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/file-manager"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpFileManagerDigestAsync(10)).Summary, entries = (await reporter.BuildCpFileManagerDigestAsync(10)).Entries, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/server-ip"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpServerIpDigestAsync(10)).Summary, addresses = (await reporter.BuildCpServerIpDigestAsync(10)).Addresses, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/debug-console"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpDebugConsoleDigestAsync(10)).Summary, files = (await reporter.BuildCpDebugConsoleDigestAsync(10)).Files, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/statistics"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpStatisticsDigestAsync(10)).Summary, rows = (await reporter.BuildCpStatisticsDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/accessories"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpAccessoriesDigestAsync(10)).Summary, rows = (await reporter.BuildCpAccessoriesDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/synonyms"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpSynonymsDigestAsync(10)).Summary, rows = (await reporter.BuildCpSynonymsDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/seo"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpSeoDigestAsync(10)).Summary, rows = (await reporter.BuildCpSeoDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/social-hub"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpSocialHubDigestAsync(10)).Summary, rows = (await reporter.BuildCpSocialHubDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/tenant-features"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpTenantFeaturesDigestAsync(10)).Summary, rows = (await reporter.BuildCpTenantFeaturesDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/customer-board"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpCustomerBoardDigestAsync(10)).Summary, rows = (await reporter.BuildCpCustomerBoardDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/fulfillment-queue"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpFulfillmentQueueDigestAsync(10)).Summary, rows = (await reporter.BuildCpFulfillmentQueueDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/sso-saml"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpSsoSamlDigestAsync(10)).Summary, rows = (await reporter.BuildCpSsoSamlDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/cp/event-bus"] = new { ok = true, surface = "cp", summary = (await reporter.BuildCpEventBusDigestAsync(10)).Summary, rows = (await reporter.BuildCpEventBusDigestAsync(10)).Rows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ...
            ["/erp/workflow-tasks"] = new { ok = true, surface = "erp", summary = (await reporter.BuildErpWorkflowTasksDigestAsync(10)).Summary, tasks = (await reporter.BuildErpWorkflowTasksDigestAsync(10)).Tasks, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/erp/vat-return"] = new { ok = true, surface = "erp", summary = (await reporter.BuildErpVatReturnDigestAsync()).Summary, source = "migration", message = "x", session, note = "contract validation" },
            ["/erp/withholding"] = new { ok = true, surface = "erp", summary = (await reporter.BuildErpWithholdingDigestAsync(10)).Summary, codes = (await reporter.BuildErpWithholdingDigestAsync(10)).Codes, txns = (await reporter.BuildErpWithholdingDigestAsync(10)).Txns, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ...
            ["/erp/aging"] = new { ok = true, surface = "erp", summary = (await reporter.BuildErpAgingDigestAsync(10)).Summary, arLabels = (await reporter.BuildErpAgingDigestAsync(10)).ArLabels, apLabels = (await reporter.BuildErpAgingDigestAsync(10)).ApLabels, inventoryLabels = (await reporter.BuildErpAgingDigestAsync(10)).InventoryLabels, arRows = (await reporter.BuildErpAgingDigestAsync(10)).ArRows, apRows = (await reporter.BuildErpAgingDigestAsync(10)).ApRows, inventoryRows = (await reporter.BuildErpAgingDigestAsync(10)).InventoryRows, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/erp/receivables"] = new { ok = true, surface = "erp", summary = (await reporter.BuildErpReceivablesDigestAsync(10)).Summary, customers = (await reporter.BuildErpReceivablesDigestAsync(10)).Customers, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ["/erp/stock-movements"] = new { ok = true, surface = "erp", summary = (await reporter.BuildErpInventoryMovementsDigestAsync(10)).Summary, movements = (await reporter.BuildErpInventoryMovementsDigestAsync(10)).Movements, count = 0, source = "migration", message = "x", session, note = "contract validation" },
            ...
            ["/erp/staff"] = Envelope("erp", "staff", await reporter.ListErpStaffAsync(10), session),
            ["/erp/contracts"] = Envelope("erp", "contracts", await reporter.ListErpContractsAsync(10), session),
            ["/erp/opening"] = Envelope("erp", "batches", await reporter.ListErpOpeningBatchesAsync(10), session),
            ["/erp/marketing"] = Envelope("erp", "campaigns", await reporter.ListErpMarketingCampaignsAsync(10), session),
            ["/erp/payroll"] = Envelope("erp", "runs", await reporter.ListErpPayrollRunsAsync(10), session),
            ["/erp/print-templates"] = Envelope("erp", "templates", await reporter.ListErpPrintTemplatesAsync(10), session),
            ["/erp/order-planning"] = Envelope("erp", "recommendations", await reporter.BuildErpOrderPlanningDigestAsync(10), session),
            ["/erp/procurement-categories"] = Envelope("erp", "categories", await reporter.BuildErpProcurementCategoriesDigestAsync(10), session),
            ["/erp/quality"] = Envelope("erp", "plans", await reporter.BuildErpQualityDigestAsync(10), session),
