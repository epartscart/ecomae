...
public sealed class LiveSurfaceLinkReporterTests
{
    ...
    public void BuildReportCataloguesSuperCpTenantAndAspNetDiagnostics()
    {
        ...
        Assert.Contains(report.NextActions, action => action.Contains("2/30 live", StringComparison.Ordinal));
        Assert.Contains(report.NextActions, action => action.Contains("cloudpanel_install_exact_route_shadow.sh /cp/users", StringComparison.Ordinal));
    }
