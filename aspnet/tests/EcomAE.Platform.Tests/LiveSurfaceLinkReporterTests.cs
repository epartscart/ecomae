...
public sealed class LiveSurfaceLinkReporterTests
{
    ...
    public void BuildReportCataloguesSuperCpTenantAndAspNetDiagnostics()
    {
        ...
        Assert.Contains(report.Links, link =>
            ...
            && link.StackToday == "aspnet");
        Assert.Equal(36, report.Links.Count(link =>
            link.HostClass == "aspnet-exact-route-shadow-live"
        ...
        Assert.Equal(48, report.Links.Count(link => link.HostClass == "aspnet-presentation-preview"));
        Assert.Contains(report.Links, link =>
            link.HostClass == "aspnet-presentation-preview"
        ...
        Assert.Contains(report.NextActions, action => action.Contains("Surface digests: wired 36", StringComparison.Ordinal));
        ...
        Assert.Contains(report.NextActions, action => action.Contains("pairsChecked=49", StringComparison.Ordinal));
