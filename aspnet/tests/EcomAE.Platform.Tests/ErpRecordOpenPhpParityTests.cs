...
public sealed class ErpRecordOpenPhpParityTests
{
    ...
    [Fact]
    public void PurchasesApp_OpenLoadsNoteVatAndSiblings()
    {
        var root = FindRepoRoot();
        var razor = File.ReadAllText(Path.Combine(root,
            "aspnet/src/EcomAE.Platform/Components/Pages/ErpPurchasesApp.razor"));
        Assert.Contains("ErpOpenedRecordBanner", razor, StringComparison.Ordinal);
        Assert.Contains("BuildErpPurchaseDetailAsync", razor, StringComparison.Ordinal);
        Assert.Contains("ReadId(ctx.Request, \"purchase_id\")", razor, StringComparison.Ordinal);
        Assert.Contains("purchase_id=", razor, StringComparison.Ordinal);
        Assert.Contains("ErpRecordOpen.Href(\"/erp/purchases-app\", \"purchase_id\"", razor, StringComparison.Ordinal);
        ...
        Assert.Contains("PhpErpModulePageHeader", razor, StringComparison.Ordinal);
        Assert.Contains("table-epc", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("uae_vat", razor, StringComparison.Ordinal);
        ...
        Assert.Equal("/erp/purchases-app?purchase_id=5#erp-row-5",
            ErpRecordOpen.Href("/erp/purchases-app", "purchase_id", 5));
        Assert.Equal(
            "/erp/purchases-app?purchase_id=5",
            ErpRecordOpen.PreserveRecordQuery(
                "/erp/purchases-app",
                "/ERP/?epc_erp_shell=1&area=purchasing&tab=purchases&purchase_id=5"));
    }
    ...
    [Fact]
    public void PayablesApp_OpenLoadsTrnCurrencyAndSiblings()
    {
        var root = FindRepoRoot();
        var razor = File.ReadAllText(Path.Combine(root,
        ...
        Assert.Contains("ReadId(ctx.Request, \"supplier_id\")", razor, StringComparison.Ordinal);
        Assert.Contains("supplier_id=", razor, StringComparison.Ordinal);
        Assert.Contains("ErpRecordOpen.Href(\"/erp/payables-app\", \"supplier_id\"", razor, StringComparison.Ordinal);
        Assert.Contains("TrnExcerpt", razor, StringComparison.Ordinal);
        Assert.Contains("CurrencyCode", razor, StringComparison.Ordinal);
        Assert.Contains("same-currency siblings", razor, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("stay Classic", razor, StringComparison.Ordinal);
        ...
        Assert.Equal("/erp/payables-app?supplier_id=6#erp-row-6",
            ErpRecordOpen.Href("/erp/payables-app", "supplier_id", 6));
        Assert.Equal(
            "/erp/payables-app?supplier_id=6",
            ErpRecordOpen.PreserveRecordQuery(
                "/erp/payables-app",
                "/ERP/?epc_erp_shell=1&area=ap&tab=payables&supplier_id=6"));
    }
    ...
    public void ModulesApp_OpenLoadsBodyExcerptAndKeepsWrites()
    {
        ...
        Assert.Contains("ReadId(ctx.Request, \"prototype_id\")", text, StringComparison.Ordinal);
        Assert.Contains("Modules.ModuleEditorAsync(", text, StringComparison.Ordinal);
        Assert.Contains("Modules.ModulesAsync(", text, StringComparison.Ordinal);
        Assert.Contains("/cp/modules/write", text, StringComparison.Ordinal);
        Assert.Contains("PhpReferenceOnlyHref(_phpTab)", text, StringComparison.Ordinal);
        Assert.Contains("module_id=", text, StringComparison.Ordinal);
        Assert.DoesNotContain("PhpParityModuleBody", text, StringComparison.Ordinal);
        Assert.DoesNotContain("BuildCpModulesDetailAsync", text, StringComparison.Ordinal);
        Assert.DoesNotContain("AspNetPrimaryHref(_phpTab)\">Open", text, StringComparison.Ordinal);
        Assert.DoesNotContain("/php-reference/", text, StringComparison.Ordinal);
        Assert.DoesNotContain("ASP.NET", text, StringComparison.Ordinal);
        Assert.DoesNotContain("@bind", text, StringComparison.Ordinal);
    ...
    public void TemplatesManagerApp_OpenLoadsDataValueExcerptAndKeepsClassicSwitch()
    {
        ...
        Assert.Contains("Templates.TemplateAsync(", razor, StringComparison.Ordinal);
        Assert.Contains("Templates.TemplatesAsync(", razor, StringComparison.Ordinal);
        Assert.Contains("Query[\"tpl_id\"]", razor, StringComparison.Ordinal);
        Assert.Contains("Query[\"template_id\"]", razor, StringComparison.Ordinal);
        Assert.Contains("/cp/templates-manager/set-current", razor, StringComparison.Ordinal);
        Assert.Contains("/cp/templates-manager/delete", razor, StringComparison.Ordinal);
        Assert.Contains("/cp/templates-manager/save", razor, StringComparison.Ordinal);
        Assert.Contains("name=\"confirmWrites\"", razor, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("PhpParityModuleBody", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("BuildCpTemplatesManagerDetailAsync", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("stay on the Classic twin", razor, StringComparison.Ordinal);
    ...
    public void PluginsManagerApp_OpenLoadsDataValueExcerptAndKeepsClassicActivate()
    {
        ...
        Assert.Contains("Plugins.PluginAsync(", razor, StringComparison.Ordinal);
        Assert.Contains("Plugins.PluginsAsync(", razor, StringComparison.Ordinal);
        Assert.Contains("Query[\"plugin_id\"]", razor, StringComparison.Ordinal);
        ...
        Assert.Contains("/cp/plugins-manager/delete", razor, StringComparison.Ordinal);
        Assert.Contains("/cp/plugins-manager/save", razor, StringComparison.Ordinal);
        Assert.Contains("name=\"confirmWrites\"", razor, StringComparison.Ordinal);
        Assert.Contains("name=\"flag_value\"", razor, StringComparison.Ordinal);
        Assert.Contains("ControlLock", razor, StringComparison.Ordinal);
        Assert.Contains("class=\"hpanel\"", razor, StringComparison.Ordinal);
        Assert.Contains("PhpReferenceOnlyHref(_phpTab)", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("PhpParityModuleBody", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("BuildCpPluginsManagerDetailAsync", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("stay on the Classic twin", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("epc-w22-hero", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("@onclick", razor, StringComparison.Ordinal);
        Assert.DoesNotContain("@onsubmit:preventDefault", razor, StringComparison.Ordinal);
    ...
    [Fact]
    public void BulkUploadApp_OpenLoadsHubDetailAndNativeActions()
    {
        var root = FindRepoRoot();
        var razor = File.ReadAllText(Path.Combine(root, "aspnet/src/EcomAE.Platform/Components/Pages/CpBulkUploadApp.razor"));
        Assert.Contains("ICpBulkUploadHubService Hub", razor, StringComparison.Ordinal);
        Assert.Contains("Hub.GetUploadAsync(_uploadId", razor, StringComparison.Ordinal);
        ...
        Assert.Contains("value=\"add_to_cart\"", razor, StringComparison.Ordinal);
        Assert.Contains("value=\"create_shop_quote\"", razor, StringComparison.Ordinal);
        Assert.Contains("value=\"create_crm_quote\"", razor, StringComparison.Ordinal);
        Assert.Contains("value=\"mark_reviewed\"", razor, StringComparison.Ordinal);
        Assert.Contains("value=\"process_upload\"", razor, StringComparison.Ordinal);
        Assert.Contains("PhpReferenceOnlyHref(_phpTab)", razor, StringComparison.Ordinal);
