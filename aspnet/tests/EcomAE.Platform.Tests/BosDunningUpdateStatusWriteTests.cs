...
public sealed class BosDunningUpdateStatusWriteTests
{
    [Fact]
    public void Route_exposes_update_status()
    {
        Assert.Equal("/bos/dunning/update-status", EcomAeRoutes.BosDunningUpdateStatus);
    }

    [Fact]
    public void Page_posts_native_update_status()
    {
        ...
        Assert.Contains("action=\"/bos/dunning/update-status\"", razor, StringComparison.Ordinal);
        ...
        Assert.Contains("name=\"notes\"", razor, StringComparison.Ordinal);
    ...
    [Fact]
    public void Catalog_marks_update_status_live_gated()
    {
        var write = SurfacePayloadContractCatalog.Functions.First(item =>
            item.AspNetRouteOrCapability == "/bos/dunning/update-status");
        Assert.Equal("write-live-gated", write.Status);
        Assert.Contains("update_status", write.Notes, StringComparison.Ordinal);
    ...
    [Fact]
    public void Program_and_module_update_status_write()
    {
        var module = File.ReadAllText(Path.Combine(FindRepoRoot(), "aspnet/src/EcomAE.Platform/Modules/BosModule.cs"));
        Assert.Contains("BosDunningUpdateStatus", module, StringComparison.Ordinal);
