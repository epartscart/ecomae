...
public sealed class BosMarketplaceUninstallWriteTests
{
    [Fact]
    public void Route_exposes_uninstall()
    {
        Assert.Equal("/bos/marketplace/uninstall", EcomAeRoutes.BosMarketplaceUninstall);
    }

    [Fact]
    public void Php_bos_site_key_matches_preg_replace()
    {
        Assert.Equal("", BosMarketplaceWriteService.PhpBosSiteKey(null));
        Assert.Equal("", BosMarketplaceWriteService.PhpBosSiteKey(""));
        Assert.Equal("acmeshop", BosMarketplaceWriteService.PhpBosSiteKey("Acme-Shop!"));
        Assert.Equal("epartscart", BosMarketplaceWriteService.PhpBosSiteKey("ePartsCart"));
    }

    [Fact]
    public void Page_posts_native_uninstall()
    {
        ...
        Assert.Contains("action=\"/bos/marketplace/uninstall\"", razor, StringComparison.Ordinal);
    ...
    [Fact]
    public void Catalog_marks_uninstall_live_gated()
    {
        var write = SurfacePayloadContractCatalog.Functions.First(item =>
            item.AspNetRouteOrCapability == "/bos/marketplace/uninstall");
        Assert.Equal("write-live-gated", write.Status);
        Assert.Contains("uninstall", write.Notes, StringComparison.Ordinal);
    ...
    [Fact]
    public void Program_and_module_uninstall_write()
    {
        var module = File.ReadAllText(Path.Combine(FindRepoRoot(), "aspnet/src/EcomAE.Platform/Modules/BosModule.cs"));
        Assert.Contains("BosMarketplaceUninstall", module, StringComparison.Ordinal);
        Assert.Contains("UninstallAsync", module, StringComparison.Ordinal);
        ...
        Assert.Contains("UPDATE `epc_marketplace_installs`", service, StringComparison.Ordinal);
