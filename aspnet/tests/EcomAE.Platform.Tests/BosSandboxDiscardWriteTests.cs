...
public sealed class BosSandboxDiscardWriteTests
{
    [Fact]
    public void Route_exposes_discard()
    {
        Assert.Equal("/bos/sandbox/discard", EcomAeRoutes.BosSandboxDiscard);
    }

    [Fact]
    public void Page_posts_native_discard()
    {
        ...
        Assert.Contains("action=\"/bos/sandbox/discard\"", razor, StringComparison.Ordinal);
    ...
    [Fact]
    public void Catalog_marks_discard_live_gated()
    {
        var write = SurfacePayloadContractCatalog.Functions.First(item =>
            item.AspNetRouteOrCapability == "/bos/sandbox/discard");
        Assert.Equal("write-live-gated", write.Status);
        Assert.Contains("discard", write.Notes, StringComparison.Ordinal);
    ...
    [Fact]
    public void Program_and_module_discard_write()
    {
        var module = File.ReadAllText(Path.Combine(FindRepoRoot(), "aspnet/src/EcomAE.Platform/Modules/BosModule.cs"));
        Assert.Contains("BosSandboxDiscard", module, StringComparison.Ordinal);
        Assert.Contains("DiscardAsync", module, StringComparison.Ordinal);
