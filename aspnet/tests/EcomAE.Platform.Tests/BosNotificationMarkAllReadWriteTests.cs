...
public sealed class BosNotificationMarkAllReadWriteTests
{
    [Fact]
    public void Route_exposes_mark_all_read()
    {
        Assert.Equal("/bos/notifications/mark-all-read", EcomAeRoutes.BosNotificationsMarkAllRead);
    }

    [Fact]
    public void Page_posts_native_mark_all_read()
    {
        ...
        Assert.Contains("action=\"/bos/notifications/mark-all-read\"", razor, StringComparison.Ordinal);
    ...
    [Fact]
    public void Catalog_marks_mark_all_read_live_gated()
    {
        var write = SurfacePayloadContractCatalog.Functions.First(item =>
            item.AspNetRouteOrCapability == "/bos/notifications/mark-all-read");
        Assert.Equal("write-live-gated", write.Status);
        Assert.Contains("mark_all_read", write.Notes, StringComparison.Ordinal);
    ...
    [Fact]
    public void Program_and_module_mark_all_read_write()
    {
        var module = File.ReadAllText(Path.Combine(FindRepoRoot(), "aspnet/src/EcomAE.Platform/Modules/BosModule.cs"));
        Assert.Contains("BosNotificationsMarkAllRead", module, StringComparison.Ordinal);
        Assert.Contains("MarkAllReadAsync", module, StringComparison.Ordinal);
        ...
        Assert.Contains("epc_notifications_mark_all_read", service, StringComparison.Ordinal);
        Assert.Contains("(`user_id` = ? OR `user_id` = 0)", service, StringComparison.Ordinal);
