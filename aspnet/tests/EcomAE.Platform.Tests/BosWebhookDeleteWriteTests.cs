...
public sealed class BosWebhookDeleteWriteTests
{
    [Fact]
    public void Route_exposes_delete()
    {
        Assert.Equal("/bos/webhooks/delete", EcomAeRoutes.BosWebhooksDelete);
    }

    [Fact]
    public void Page_posts_native_delete()
    {
        ...
        Assert.Contains("action=\"/bos/webhooks/delete\"", razor, StringComparison.Ordinal);
        Assert.Contains("name=\"confirmWrites\"", razor, StringComparison.Ordinal);
        Assert.Contains("value=\"true\"", razor, StringComparison.Ordinal);
        Assert.Contains("name=\"webhook_id\"", razor, StringComparison.Ordinal);
    ...
    public void Catalog_marks_delete_live_gated()
    {
        var write = SurfacePayloadContractCatalog.Functions.First(item =>
            item.AspNetRouteOrCapability == "/bos/webhooks/delete");
        Assert.Equal("write-live-gated", write.Status);
        Assert.Contains("delete", write.Notes, StringComparison.Ordinal);
        Assert.Contains("epc_webhooks", write.Notes, StringComparison.Ordinal);
    ...
    [Fact]
    public void Program_and_module_delete_write()
    {
        var module = File.ReadAllText(Path.Combine(FindRepoRoot(), "aspnet/src/EcomAE.Platform/Modules/BosModule.cs"));
        Assert.Contains("BosWebhooksDelete", module, StringComparison.Ordinal);
        Assert.Contains("DeleteAsync", module, StringComparison.Ordinal);
        ...
        Assert.Contains("IBosWebhookWriteService", program, StringComparison.Ordinal);
        var service = File.ReadAllText(Path.Combine(FindRepoRoot(), "aspnet/src/EcomAE.Platform/Bos/BosWebhookWriteService.cs"));
        Assert.Contains("epc_webhooks_delete", service, StringComparison.Ordinal);
        ...
        Assert.Contains("\"webhooks\"", policy, StringComparison.Ordinal);
    }
