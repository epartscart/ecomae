...
public sealed class BosWorkflowToggleWriteTests
{
    [Fact]
    public void Route_exposes_toggle()
    {
        Assert.Equal("/bos/workflows/toggle", EcomAeRoutes.BosWorkflowsToggle);
    }

    [Fact]
    public void Php_posted_bool_matches_cast()
    {
        Assert.False(BosWorkflowWriteService.PhpPostedBool(null));
        Assert.False(BosWorkflowWriteService.PhpPostedBool(""));
        Assert.True(BosWorkflowWriteService.PhpPostedBool("0"));
        Assert.True(BosWorkflowWriteService.PhpPostedBool("1"));
    }
    ...
    public void Page_posts_native_toggle()
    {
        ...
        Assert.Contains("action=\"/bos/workflows/toggle\"", razor, StringComparison.Ordinal);
    ...
    public void Catalog_marks_toggle_live_gated()
    {
        var write = SurfacePayloadContractCatalog.Functions.First(item =>
            item.AspNetRouteOrCapability == "/bos/workflows/toggle");
        Assert.Equal("write-live-gated", write.Status);
        Assert.Contains("toggle", write.Notes, StringComparison.Ordinal);
    ...
    public void Program_and_module_toggle_write()
    {
        ...
        Assert.Contains("BosWorkflowsToggle", module, StringComparison.Ordinal);
        ...
        Assert.Contains("UPDATE `epc_workflows`", service, StringComparison.Ordinal);
