...
/// <summary>
/// Safe dry-run validator for SEO sitemap warm replacement. HTTP fetches/writes are always blocked.
/// </summary>
public sealed class SeoSitemapWarmDryRunExecutor : IMigrationWorkerJobDryRunExecutor
{
    public bool CanExecute(MigrationWorkerJob job) =>
        string.Equals(job.Key, "seo-sitemap-warm", StringComparison.OrdinalIgnoreCase);
    ...
    public MigrationWorkerJobDryRunOutput Execute(MigrationWorkerJob job, MigrationWorkerJobRunRequest request)
    {
        ...
        {
            const string warning = "Provide parameters.sample_urls as warm targets (one URL per line).";
        ...
        var summary = valid > 0
            ? $"Validated {valid} warm URL(s); HTTP warm fetches blocked."
