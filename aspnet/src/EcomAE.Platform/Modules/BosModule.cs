...
public sealed class BosModule : ISurfaceModule
{
    ...
    public void MapEndpoints(IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPost(EcomAeRoutes.BosBillingSubscribe, async (
            HttpContext context,
            ILegacySessionValidator validator,
            IBosBillingWriteService writes,
            CancellationToken cancellationToken) =>
        {
            ...
            if (session.Kind != LegacySessionKind.Admin || !session.Capabilities.Contains("bos"))
            {
                return LiveWriteFormBinder.LoginRedirect(context, "/bos/login?returnUrl=/bos/fleet-summary-app", "Admin BOS capability required for billing subscribe.");
            }
            ...
            var body = await LiveWriteFormBinder.ReadJsonOrDefaultAsync<BosBillingSubscribeBody>(context, cancellationToken)
                       ?? new();
            var siteKey = body.SiteKey;
            ...
            if (!confirm)
            {
                return Results.Ok(new
                {
                    ...
                    phpAuthoritative = true,
                    validation_code = "dry_run",
                    message = "Set confirmWrites=true to subscribe a tenant.",
                    session = SessionPayload(session)
                });
            }

            var written = await writes.SubscribeAsync(siteKey, planId, cancellationToken);
            return LiveWriteFormBinder.Complete(
                context,
                "/bos/fleet-summary-app",
                written.Succeeded,
        ...
        endpoints.MapPost(EcomAeRoutes.BosTenantConfigSet, async (
            HttpContext context,
            ILegacySessionValidator validator,
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            if (session.Kind != LegacySessionKind.Admin || !session.Capabilities.Contains("bos"))
            {
                return LiveWriteFormBinder.LoginRedirect(context, "/bos/login?returnUrl=/bos/fleet-summary-app", "Admin BOS capability required for tenant-config set.");
            }
            ...
            var body = await LiveWriteFormBinder.ReadJsonOrDefaultAsync<BosTenantConfigSetBody>(context, cancellationToken)
                       ?? new();
            var siteKey = body.SiteKey;
            ...
            if (!confirm)
            {
                return Results.Ok(new
                {
                    ...
                    phpAuthoritative = true,
                    validation_code = "dry_run",
                    message = "Set confirmWrites=true to save tenant config.",
                    session = SessionPayload(session)
                });
            }

            var written = await writes.SetAsync(siteKey, group, key, value, updatedBy, cancellationToken);
            return LiveWriteFormBinder.Complete(
                context,
                "/bos/fleet-summary-app",
                written.Succeeded,
    ...
    private sealed record BosBillingSubscribeBody(
        bool ConfirmWrites = false,
        string? SiteKey = null,
        long PlanId = 0);
    ...
    private sealed record BosTenantConfigSetBody(
        bool ConfirmWrites = false,
        string? SiteKey = null,
        string? Group = null,
        string? Key = null,
