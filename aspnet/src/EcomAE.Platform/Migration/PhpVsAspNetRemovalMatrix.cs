...
public static class PhpVsAspNetRemovalMatrix
{
    ...
    [
        ...
        Row("cp-catalogue", "cp", "cp/content/shop/catalogue", "/CP/shop/catalogue/products", "/cp/product-catalogue-app", "aspnet-digest", "aspnet", "shop_catalogue_products min_limit / min_limit_enable UPDATEs and shop_catalogue_categories_templates DELETE are ASP.NET-live. Tree / SKU / media / template create stay PHP."),
        ...
        Row("cp-workshop", "cp", "cp/content/shop/workshop", "/CP/shop/workshop", "/cp/workshop-app", "aspnet-digest", "aspnet", "assign / save_bay / save_tech / set_status are ASP.NET-live. Seed, create-job, lines, and appointments stay PHP."),
        ...
        Row("cp-crosses", "cp", "cp/content/shop/crosses", "/CP/shop/crosses", "/cp/crosses-app", "aspnet-digest", "aspnet", "save_crosses UPDATE and del_crosses DELETE are ASP.NET-live when all four fields are supplied. Add, brand auto-resolve, and search-delete stay PHP."),
        ...
        Row("cp-storages", "cp", "cp/content/shop/logistics/groups", "/CP/shop/logistics/storages", "/cp/storages-app", "aspnet-digest", "aspnet", "shop_storages_groups add/delete are ASP.NET-live. Warehouse create stays PHP."),
        Row("cp-quote-requests", "cp", "cp/content/shop/quote_requests", "/CP/shop/quote-requests", "/cp/quote-requests-app", "aspnet-digest", "aspnet", "admin_note UPDATE and send_quote status UPDATE are ASP.NET-live. Line quoting stays PHP."),
        Row("cp-vendor-approvals", "cp", "cp/content/users/epc_vendor_approvals.php", "/CP/users/vendor_approvals", "/cp/users-app", "aspnet-digest", "aspnet", "Vendor suspend/reject UPDATEs are ASP.NET-live. Approve/warehouse provision stays PHP."),
        ...
        Row("cp-content-pages", "cp", "cp/content/content/content_manager.php", "/CP/content/content_manager", "/cp/pages-app", "aspnet-digest", "aspnet", "Single-id published_flag and main_flag are ASP.NET-live. Body editor and system-content config stay PHP."),
        Row("erp-offices-cash", "erp", "cp/content/shop/logistics/offices_cash.php + offices_cash_editor.php", "/CP/shop/cash", "/erp/cash-accounts-app", "aspnet-digest", "aspnet", "Office cash entry INSERT and unused code DELETE are ASP.NET-live. Code add (save_custom_translation) stays PHP."),
        Row("erp-wms-locations", "erp", "content/shop/finance/epc_erp_wms.php epc_wms_location_delete + epc_wms_wave_release", "/ERP/?epc_erp_shell=1&area=warehouse&tab=wms", "/cp/warehouse-wms-app", "aspnet-digest", "aspnet", "WMS location DELETE and wave release UPDATE are ASP.NET-live. Wave create, pick add, work complete, and schema ensure stay PHP."),
        ...
        Row("erp-workflow", "erp", "content/shop/finance/epc_erp_staff.php epc_erp_workflow_update_status", "/ERP/?epc_erp_shell=1&area=overview&tab=workflow", "/erp/workflow-app", "aspnet-digest", "aspnet", "Workflow task status UPDATE is ASP.NET-live. Task create stays PHP."),
        Row("erp-collections-cases", "erp", "content/shop/finance/epc_erp_collections.php epc_coll_case_set_status", "/ERP/?epc_erp_shell=1&area=finance&tab=collections", "/cp/collections-dunning-app", "aspnet-digest", "aspnet", "Collections case status UPDATE is ASP.NET-live. Case save, promise, activity, dunning run, and hold stay PHP."),
        Row("erp-procurement-reqs", "erp", "content/shop/finance/epc_erp_procurement.php epc_proc_req_submit/decision/convert", "/ERP/?epc_erp_shell=1&area=purchasing&tab=purchase_requisitions", "/cp/purchase-requests-app", "aspnet-digest", "aspnet", "Requisition submit, approve/reject, and convert-to-PO are ASP.NET-live. Save, add-line, and schema ensure stay PHP."),
        Row("erp-ins-claims", "erp", "content/shop/finance/epc_erp_insurance.php epc_ins_claim_set_status + epc_ins_doc_delete", "/ERP/?epc_erp_shell=1&area=risk&tab=insurance", "/cp/insurance-compliance-app", "aspnet-digest", "aspnet", "Insurance claim status UPDATE and document DELETE are ASP.NET-live. Policy save/delete, doc add, and claim add stay PHP."),
        Row("erp-hr-leave-expense", "erp", "content/shop/finance/epc_erp_hr.php epc_hr_leave_set_status + epc_hr_expense_set_status", "/ERP/?epc_erp_shell=1&area=people&tab=hr", "/cp/hr-overview-app", "aspnet-digest", "aspnet", "HR leave and expense status UPDATEs are ASP.NET-live. Leave request, expense save, and schema ensure stay PHP."),
        Row("erp-cons-deletes", "erp", "content/shop/finance/epc_erp_consolidation.php epc_cons_entity_delete + epc_cons_ic_delete", "/ERP/?epc_erp_shell=1&area=consolidations&tab=consolidation_bu", "/cp/consolidations-app", "aspnet-digest", "aspnet", "Consolidation entity and IC DELETEs are ASP.NET-live. Save, figures, and schema ensure stay PHP."),
        Row("erp-vat-refund-status", "erp", "content/shop/finance/epc_bos_vat_refund.php epc_bos_vat_refund_set_status", "/ERP/?epc_erp_shell=1&area=tax&tab=vat_refund", "/erp/vat-app?tab=vat_refund", "aspnet-digest", "aspnet", "BOS VAT refund status UPDATE is ASP.NET-live. Refund save and schema ensure stay PHP."),
        ...
        Row("erp-bos-compliance-disable", "erp", "content/shop/finance/epc_bos_compliance.php epc_bos_compliance_disable_obligation", "/ERP/?epc_erp_shell=1&area=tax&tab=compliance", "/cp/soc2-compliance-app", "aspnet-digest", "aspnet", "BOS compliance obligation disable UPDATE is ASP.NET-live. Add, file, and retention stay PHP."),
        ...
        Row("erp-wht-settle", "erp", "content/shop/finance/epc_erp_withholding.php epc_wht_settle", "/ERP/?epc_erp_shell=1&area=tax&tab=withholding", "/erp/withholding-app", "aspnet-digest", "aspnet", "Withholding settle UPDATE is ASP.NET-live. Code save, record, and certificate minting stay PHP."),
        Row("erp-docx-delete", "erp", "content/shop/finance/epc_erp_doc_expiry.php epc_docx_delete", "/ERP/?epc_erp_shell=1&area=risk&tab=doc_expiry", "/cp/doc-expiry-app", "aspnet-digest", "aspnet", "Document-register DELETE (reminders + row) is ASP.NET-live. Save and reminder run stay PHP."),
        Row("erp-qm-ncr-update", "erp", "content/shop/finance/epc_erp_quality.php epc_qm_ncr_update", "/ERP/?epc_erp_shell=1&area=operations&tab=quality", "/erp/quality-app", "aspnet-digest", "aspnet", "NCR status/disposition UPDATE is ASP.NET-live. Plan save, inspection create, and NCR create stay PHP."),
        Row("erp-cs-submit", "erp", "content/shop/finance/epc_custom_shipping.php epc_cs_submit_declaration", "/ERP/?epc_erp_shell=1&area=logistics&tab=custom_shipping", "/cp/fulfillment-queue-app?tab=custom_shipping", "aspnet-digest", "aspnet", "Custom-shipping declaration submit UPDATE is ASP.NET-live. Save, delete, and PDF import stay PHP."),
        ...
        Row("erp-inventory", "erp", "erp_tabs_inventory.php", "/ERP/?epc_erp_shell=1&area=inventory&tab=inventory", "/erp/inventory-stock-app", "aspnet-digest", "aspnet", "inv_set_reorder_level UPDATE is ASP.NET-live. Movements, transfers, and closing stay PHP."),
        Row("erp-workspace-favorites", "erp", "ajax_erp.php erp_fav_add/remove", "/ERP/?epc_erp_shell=1&area=overview&tab=favorites", "/erp/workspace-favorites-app", "aspnet-digest", "aspnet", "Favourites add/remove and shortcut delete/delete-key/reset are ASP.NET-live. Shortcut add/reorder stay PHP."),
        Row("erp-jw-repairs", "erp", "ajax_erp.php jw_repair_update_status", "/ERP/?epc_erp_shell=1&area=service_mgmt&tab=jw_repairs", "/cp/jewellery-repairs-app", "aspnet-digest", "aspnet", "epc_erp_jw_repairs status UPDATE is ASP.NET-live. Create and sample seed stay PHP."),
