...
public static class PhpVsAspNetRemovalMatrix
{
    ...
    [
        ...
        Row("sf-checkout", "storefront", "content/shop/order_process", "/shop/checkout", "/storefront/checkout-app", "aspnet-digest", "php", "Signed-in and guest checkout/create are ASP.NET-live (session cookie + phone_not_auth). Guest success dest is /storefront/guest-order-app. Continue to pay is GET /storefront/payment-app. Demo pay create/notify are ASP.NET-live. Guest payment create and staff email stay Classic."),
        ...
        Row("sf-profile", "storefront", "content/users/profileform.php", "/users/profile", "/storefront/profile-app", "aspnet-digest", "aspnet", "users_profiles UPSERT, password md5(secret_succession), and confirm_contact (email/phone code) are ASP.NET-live. Contact-change send stays Classic."),
        ...
        Row("cp-procurement", "cp", "cp/content/shop/procurement", "/CP/shop/procurement/procurement", "/cp/procurement-app", "aspnet-digest", "aspnet", "procurement_main twin (dashboard / suppliers / purchase bills / payments / advances / fulfillment / warehouses / guide) over /cp/procurement/ajax. create_supplier / update_supplier / sync_suppliers / create_purchase / supplier_payment / record_advance / purchase_from_order / supplier_settlement / purchase_adjustment are ASP.NET-live via the ERP purchase, cash and inventory services."),
        Row("cp-workshop", "cp", "cp/content/shop/workshop", "/CP/shop/workshop", "/cp/workshop-app", "aspnet-digest", "aspnet", "workshop_main_page twin (board / jobs / check-in / schedule / bays & techs / guide) with the epc_workshop.js job-card popup over /cp/workshop/terminal-ajax. assign / save_bay / save_tech / set_status / create_job / add_line / create_appointment / convert_appointment / get_job / list_jobs / list_appointments / seed_demo are ASP.NET-live."),
        ...
        Row("cp-prices-upload", "cp", "cp/content/shop/prices_upload", "/CP/shop/prices_upload", "/cp/prices-upload-app", "aspnet-digest", "aspnet", "ajax_6_complete_session last_updated / records_count, min_price_acl_save UPSERT, storefront_temp_disabled toggle, and vendor_code_save are live. CSV import, file ingest, extract, and sitemap stay PHP."),
        ...
        Row("cp-price-storage-rules", "cp", "cp/content/shop/pricing/epc_pm_storage_panel.php", "/CP/shop/pricing", "/cp/price-lists-app", "aspnet-digest", "aspnet", "Price-storage rule save (ON DUPLICATE KEY) and DELETE are ASP.NET-live."),
        ...
        Row("cp-currencies", "cp", "content/shop/finance/nastrojka-kursov-valyut.php", "/CP/shop/finance/nastrojka-kursov-valyut", "/cp/currencies-app", "aspnet-digest", "aspnet", "shop_currencies.rate and available flag UPDATEs are ASP.NET-live. Nightly FX schedule_save writes on /cp/currencies/schedule-save. Live FX pull stays PHP."),
        ...
        Row("cp-live-fx", "cp", "content/shop/finance/epc_currency_live_rates.php", "/CP/shop/finance/epc_currency_live_rates", "/cp/currencies-app", "aspnet-routed", "php", "schedule_save writes on /cp/currencies/schedule-save. Live FX pull stays PHP."),
        ...
        Row("erp-multi-entity", "erp", "content/shop/finance/epc_multi_entity.php", "/CP/shop/finance/epc_multi_entity", "/erp/multi-entity-app", "aspnet-digest", "aspnet", "epc_entity_create_group / add_member / record_intercompany / eliminate are ASP.NET-live. Schema ensure and consolidated TB stay PHP. ajax_erp multi_entity_save is ASP.NET-live."),
        Row("erp-multi-currency-gl", "erp", "content/shop/finance/epc_multi_currency_gl.php + epc_ccy_set_rate", "/CP/shop/finance/epc_multi_currency_gl", "/erp/multi-currency-gl-app", "aspnet-digest", "aspnet", "epc_mcgl_set_rate UPSERT is ASP.NET-live. Revaluation, journal entry, and seed stay PHP."),
        ...
        Row("erp-projects", "erp", "content/shop/finance/epc_erp_projects.php epc_prj_save", "/ERP/?epc_erp_shell=1&area=projects&tab=projects", "/erp/projects-overview-app", "aspnet-digest", "aspnet", "Project save INSERT/UPDATE is ASP.NET-live. Task save, timesheet log, and schema ensure stay PHP."),
        Row("erp-payroll", "erp", "content/shop/finance/epc_erp_payroll.php epc_erp_payroll_generate_run + epc_erp_payroll_approve_run + epc_erp_payroll_update_line_days + epc_erp_payroll_pay_run + hr_update_days", "/CP/shop/finance/epc_wps_payroll", "/erp/payroll-app", "aspnet-digest", "aspnet", "Payroll generate INSERT/rebuild, approve UPDATE, line-days recalc, pay (cash entry + paid status), and hr_update_days are ASP.NET-live. Schema ensure and COA GL stay PHP."),
        Row("erp-subscriptions", "erp", "content/shop/finance/epc_erp_subscriptions.php epc_sub_save + epc_sub_set_status + epc_sub_generate_invoice + sub_invoice_paid", "/ERP/?epc_erp_shell=1&area=sales&tab=subscriptions", "/erp/sales-orders-app?tab=subscriptions", "aspnet-digest", "aspnet", "Subscription save INSERT/UPDATE, status UPDATE, cycle-invoice generate, and invoice-paid UPDATE are ASP.NET-live. Schema ensure stays PHP."),
        Row("erp-contracts", "erp", "content/shop/finance/epc_erp_contracts.php epc_ctr_save + epc_ctr_set_status + epc_ctr_sign + epc_ctr_ocr_store", "/ERP/?epc_erp_shell=1&area=sales&tab=contracts", "/erp/contracts-app", "aspnet-digest", "aspnet", "Contract save INSERT/UPDATE, status UPDATE, e-sign INSERT, and OCR text UPDATE are ASP.NET-live. Schema ensure stays PHP."),
        Row("erp-workflow", "erp", "content/shop/finance/epc_erp_staff.php epc_erp_workflow_update_status + epc_erp_workflow_create", "/ERP/?epc_erp_shell=1&area=overview&tab=workflow", "/erp/workflow-app", "aspnet-digest", "aspnet", "Department workflow board matches Classic twin (dept filter, Add task, per-row Done/Start). Create INSERT and status UPDATE are ASP.NET-live. Staff schema ensure and sample seed stay PHP."),
        ...
        Row("erp-ins-claims", "erp", "content/shop/finance/epc_erp_insurance.php epc_ins_claim_save + epc_ins_claim_set_status + epc_ins_doc_delete + epc_ins_save + epc_ins_doc_add + epc_ins_delete", "/ERP/?epc_erp_shell=1&area=risk&tab=insurance", "/erp/insurance-compliance-app", "aspnet-digest", "aspnet", "Insurance claim save, claim status UPDATE, document DELETE, policy save, policy delete, and document add are ASP.NET-live. Expiry upsert/sync, file bytes, and schema ensure stay PHP."),
        Row("erp-hr-leave-expense", "erp", "content/shop/finance/epc_erp_hr.php epc_hr_employee_save + epc_hr_attendance_log + epc_hr_leave_request + epc_hr_expense_save + epc_hr_leave_set_status + epc_hr_expense_set_status + epc_hr_payroll_run", "/ERP/?epc_erp_shell=1&area=people&tab=hr", "/erp/hr-overview-app", "aspnet-digest", "aspnet", "HR employee save, attendance upsert, leave request INSERT, expense save INSERT, leave/expense status UPDATEs, and HR payroll generate are ASP.NET-live. Schema ensure stays PHP."),
        Row("erp-projects", "erp", "content/shop/finance/epc_erp_projects.php epc_prj_save + epc_prj_task_save + epc_prj_log_time", "/ERP/?epc_erp_shell=1&area=projects&tab=projects", "/erp/projects-overview-app", "aspnet-digest", "aspnet", "Project save, task save, and timesheet log are ASP.NET-live. Schema ensure stays PHP."),
        Row("erp-cons-deletes", "erp", "content/shop/finance/epc_erp_consolidation.php epc_cons_entity_save + epc_cons_entity_delete + epc_cons_ic_delete + epc_cons_figures_save + epc_cons_ic_save", "/ERP/?epc_erp_shell=1&area=consolidations&tab=consolidation_bu", "/erp/consolidations-app", "aspnet-digest", "aspnet", "Consolidation entity save, entity DELETE, IC DELETE, figures UPSERT, and IC save INSERT are ASP.NET-live. Schema ensure stays PHP."),
        ...
        Row("erp-pf-case-cancel", "erp", "content/shop/finance/epc_erp_processflow.php epc_pf_case_cancel + epc_pf_step_delete + epc_pf_set_dept_head + epc_pf_case_reassign + epc_pf_process_save + epc_pf_step_save + epc_pf_case_start + epc_pf_case_act", "/ERP/?epc_erp_shell=1&area=overview&tab=processflow", "/erp/process-flow-tasks-app", "aspnet-digest", "aspnet", "Process-flow monitor/inbox/org tabs match Classic twin presentation. Case cancel UPDATE, step DELETE, department-head UPSERT, reassign, process save, step save, case start INSERT, and case act UPDATE are ASP.NET-live. Seed and schema ensure stay PHP."),
        Row("erp-bos-wf-disable-rule", "erp", "content/shop/finance/epc_bos_workflow.php epc_bos_wf_disable_rule + epc_bos_wf_save_rule + epc_bos_wf_decide + epc_bos_wf_raise", "/ERP/?epc_erp_shell=1&area=overview&tab=approvals", "/erp/approvals-app", "aspnet-digest", "aspnet", "BOS approval-rule save INSERT/UPDATE, disable UPDATE, decide UPDATE, and request raise INSERT are ASP.NET-live. Schema ensure stays PHP."),
        Row("erp-bos-compliance-disable", "erp", "content/shop/finance/epc_bos_compliance.php epc_bos_compliance_disable_obligation + epc_bos_compliance_add_obligation + epc_bos_compliance_set_filing + epc_bos_retention_save", "/ERP/?epc_erp_shell=1&area=tax&tab=compliance", "/erp/soc2-compliance-app", "aspnet-digest", "aspnet", "BOS compliance obligation add UPSERT, disable UPDATE, filing UPSERT, and retention UPSERT are ASP.NET-live. Schema ensure stays PHP."),
        Row("erp-fy-reopen-period", "erp", "content/shop/finance/epc_erp_closing.php epc_fy_reopen_year + epc_fy_set_period_status + epc_fy_create_year + epc_erp_fiscal_set_lock + content/shop/finance/epc_erp_period_close.php epc_erp_period_soft_close", "/ERP/?epc_erp_shell=1&area=finance&tab=year_end", "/erp/period-close-app", "aspnet-digest", "aspnet", "Fiscal-year create INSERT, reopen, period status UPDATEs, fiscal lock, and period soft-close are ASP.NET-live. Year-end close and schema ensure stay PHP."),
        Row("erp-fin-period-status", "erp", "content/shop/finance/epc_erp_fin_advanced.php epc_fin_period_set_status + epc_fin_periods_generate + epc_fin_alloc_rule_save", "/ERP/?epc_erp_shell=1&area=cost_acct&tab=fin_advanced", "/erp/fin-advanced-app", "aspnet-digest", "aspnet", "Advanced finance period status UPDATE, period generate INSERT, and alloc-rule save are ASP.NET-live. Alloc run, accrual save, FX revalue, and schema ensure stay PHP."),
        ...
        Row("erp-mfg-wo", "erp", "content/shop/finance/epc_erp_manufacturing.php epc_mfg_wo_create", "/ERP/?epc_erp_shell=1&area=production&tab=manufacturing", "/erp/production-overview-app", "aspnet-digest", "aspnet", "Manufacturing work-order create INSERT is ASP.NET-live. BOM save, WO issue/complete, and schema ensure stay PHP."),
        Row("erp-mfg-bom", "erp", "content/shop/finance/epc_erp_manufacturing.php epc_mfg_bom_save", "/ERP/?epc_erp_shell=1&area=production&tab=manufacturing", "/erp/production-overview-app", "aspnet-digest", "aspnet", "Manufacturing BOM save INSERT/UPDATE + line replace is ASP.NET-live. WO create/issue/complete and schema ensure stay PHP."),
    ...
    public static IReadOnlyDictionary<string, object> BuildReport()
    {
        ...
        return new Dictionary<string, object>
        {
            ...
            ["closedThisWave"] = new[]
            {
                ...
                "Payroll generate → /erp/payroll/generate live",
                "Payroll approve → /erp/ajax/payroll-approve live",
                "Payroll pay → /erp/payroll/pay live",
                "Payroll line days → /erp/payroll/update-days live",
