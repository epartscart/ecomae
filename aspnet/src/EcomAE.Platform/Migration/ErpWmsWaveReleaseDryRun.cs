...
/// <summary>Wave B dry-run for PHP <c>wms_wave_release</c>. Never UPDATE. PHP authoritative.</summary>
...
public sealed class ErpWmsWaveReleaseDryRun : IErpWmsWaveReleaseDryRun
{
    public ErpWmsWaveReleaseDryRunResult Evaluate(ErpWmsWaveReleaseRequest request)
    {
        ...
        if (request.ConfirmWrites)
            return Refuse("dry-run-confirm-refused","confirm_writes_refused","confirm_writes requested but live ASP.NET wms_wave_release is not implemented; PHP ajax_erp.php remains authoritative.", request);
        if (request.Id <= 0)
        ...
        return new("dry-run-validated",0,true,false,true,"ok",true,request.Id,
            ...
            "WMS wave release payload validated; UPDATE blocked.",
    ...
    private static ErpWmsWaveReleaseDryRunResult Refuse(string s,string c,string d,ErpWmsWaveReleaseRequest r)=>
        new(s,0,true,false,true,c,false,r.Id,[],d,"/CP/content/shop/finance/erp/ajax_erp.php?action=wms_wave_release");
}
