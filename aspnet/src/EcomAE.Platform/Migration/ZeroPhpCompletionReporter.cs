...
public sealed class ZeroPhpCompletionReporter : IZeroPhpCompletionReporter
{
    public ZeroPhpCompletionReport BuildReport()
    {
        ...
        [
            ...
            new("CP, ERP, BOS, and tenant workflow parity", 25, 28, "shell-summaries-started", [
            ...
            new("Storefront and public API parity", 15, 92, "price-and-catalog-cache-routes-started", [
                "Price lookup plus catalog cache/DB routes including articles/engine/suppliers/brand-parts have readers + API-key auth.",
                "Live UMAPI proxy fills for non-cacheable articles/engine, staging smoke, and exact-route shadows remain."
            ]),
            new("Background jobs and scheduled work", 10, 68, "dry-run-validators-started", [
                "Dry-run validators cover price-import, sitemap, backups, notifications, erp-reports, currency, demo-expire, platform-jobs, seo ping/warm, uae-tax, and apai jobs (writes blocked).",
                "Batch 1 still requires per-job parity samples and live smoke before schedule cutover."
            ]),
            new("Data, auth, observability, and rollback evidence", 15, 78, "auth-claims-wired-evidence-pending", [
                "API-key auth/quota path is wired for price lookup and catalog cache/DB routes against epc_api_clients.",
                "Admin sessions require users_groups_bind ∩ groups.for_backend; customer sessions validated; shells reject anonymous.",
                "UMAPI usage, platform-jobs, and CP/ERP/BOS summary diagnostics are read-only.",
