...
/// <summary>Wave B dry-run for PHP <c>po_status</c>. Never UPDATE. PHP authoritative.</summary>
...
public sealed class ErpPoStatusDryRun : IErpPoStatusDryRun
{
    public ErpPoStatusDryRunResult Evaluate(ErpPoStatusRequest request)
    {
        ...
        if (request.ConfirmWrites)
            return Refuse("dry-run-confirm-refused","confirm_writes_refused","confirm_writes requested but live ASP.NET po_status is not implemented; PHP ajax_erp.php remains authoritative.", request);
        if (request.Id <= 0)
        ...
        return new("dry-run-validated",0,true,false,true,"ok",true,request.Id, request.TargetStatus,
            ...
            "ERP po_status payload validated; UPDATE blocked.",
    ...
    private static ErpPoStatusDryRunResult Refuse(string s,string c,string d,ErpPoStatusRequest r)=>
        new(s,0,true,false,true,c,false,r.Id, r.TargetStatus,[],d,"/CP/content/shop/finance/erp/ajax_erp.php?action=po_status");
}
