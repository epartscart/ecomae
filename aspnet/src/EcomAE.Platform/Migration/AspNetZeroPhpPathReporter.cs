...
public sealed class AspNetZeroPhpPathReporter : IAspNetZeroPhpPathReporter
{
    public AspNetZeroPhpPathReport BuildReport()
    {
        ...
        [
            ...
            new("4-function-parity", "Interactive module writes/menus/flows", "in-progress", "aspNetInteractiveComplete=0; full ajax_erp.php catalog (321) all dedicated; BOS ajax_epc_bos catalog; CP/storefront module ajax+classic form catalog (321 actions incl. Wave F holdouts + classic forms) + POS/portal/on-prem pack dry-runs; on-premises scaffolds; write-dryrun dual-sample operator floor; live writes still PHP."),
        ...
        return new AspNetZeroPhpPathReport(
            ...
            [
                ...
                "Dual-sample CP/storefront module ajax + classic_form catalogs vs PHP; richer dedicated validation; never invent cutoverAllowed.",
            ],
            Notes:
            [
