...
public sealed class AspNetZeroPhpPathReporter : IAspNetZeroPhpPathReporter
{
    public AspNetZeroPhpPathReport BuildReport()
    {
        ...
        [
            ...
            new("2-scaffold", "ASP.NET digests + hybrid shells", "complete", "128 surface digests + storefront digests + ~155 presentation apps on www."),
            new("3-presentation-parity", "Same-to-same chrome (fonts/CSS/heroes/menus)", "in-progress", "Marketing core+solutions scaffolds (platform/about/faq/pricing/contact/industries/capabilities/demo); CP/ERP/BOS/storefront hybrid on www; tenants PHP-primary under parity gate."),
            new("4-function-parity", "Interactive module writes/menus/flows", "in-progress", "aspNetInteractiveComplete=0; cart + garage notepad + OMS + ERP cash/GL/purchase/invoice dry-runs; live writes still PHP."),
        ...
        return new AspNetZeroPhpPathReport(
            ...
            HonestCompletionPct: 52,
            Phases: phases,
            NextBuilds:
            [
                "Dual-sample marketing scaffolds vs live PHP → exact-route candidates.",
                "Dual-sample cart + garage notepad write dry-runs vs PHP ajax, then promote.",
                "Dual-sample OMS + ERP void/cancel/reverse dry-runs vs PHP ajax, then promote.",
                "Free-tools/platform-guides scaffolds; per-tenant storefront theme parity (epartscart first).",
