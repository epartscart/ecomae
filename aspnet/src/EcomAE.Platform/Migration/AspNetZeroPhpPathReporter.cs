...
public sealed class AspNetZeroPhpPathReporter : IAspNetZeroPhpPathReporter
{
    public AspNetZeroPhpPathReport BuildReport()
    {
        ...
        [
            ...
            new("4-function-parity", "Interactive module writes/menus/flows", "in-progress", "aspNetInteractiveComplete=0; cart/quote/garage/checkout/OMS/ERP cash create+receipt+payment + on-premises dry-runs; write-dryrun dual-sample operator floor; live writes still PHP."),
        ...
        return new AspNetZeroPhpPathReport(
            ...
            [
                ...
                "Paired PHP ajax vs ASP.NET write dry-run samples (checkout/OMS update-item/pay-refund) via write-dryrun operator.",
                "Dual-sample ERP cash create/receipt/payment + checkout/OMS dry-runs; then staged exact-route.",
                "ASP.NET Core on-prem installer pack; more OMS/ERP write coverage until interactive parity.",
            ],
            Notes:
            [
