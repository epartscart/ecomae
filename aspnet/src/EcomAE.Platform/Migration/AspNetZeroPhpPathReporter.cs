...
public sealed class AspNetZeroPhpPathReporter : IAspNetZeroPhpPathReporter
{
    public AspNetZeroPhpPathReport BuildReport()
    {
        ...
        [
            ...
            new("4-function-parity", "Interactive module writes/menus/flows", "in-progress", "aspNetInteractiveComplete=0; cart qty + checked_for_order + OMS set_item_status dry-runs; live writes still PHP."),
        ...
        return new AspNetZeroPhpPathReport(
            ...
            HonestCompletionPct: 44,
            Phases: phases,
            NextBuilds:
            [
                "Dual-sample /marketing/app (hub + home sections) vs live PHP / → exact-route marketing cutover candidates.",
                "Dual-sample cart dry-runs (change-count-need + check-for-order) vs PHP ajax, then promote type-2 writes.",
                "Dual-sample OMS set-item-status dry-run vs ajax_epc_orders_oms.php, then promote item status writes.",
