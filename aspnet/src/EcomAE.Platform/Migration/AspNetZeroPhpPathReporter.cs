...
public sealed class AspNetZeroPhpPathReporter : IAspNetZeroPhpPathReporter
{
    public AspNetZeroPhpPathReport BuildReport()
    {
        ...
        [
            ...
            new("4-function-parity", "Interactive module writes/menus/flows", "in-progress", "aspNetInteractiveComplete=0; full ajax_erp.php catalog (321) all dedicated; BOS ajax_epc_bos catalog; CP module ajax catalog (procurement/document_control/customer_mgmt/auto_price/CRM) + POS/portal/on-prem pack dry-runs; storefront/CP leftover ajax dry-runs; on-premises scaffolds; write-dryrun dual-sample operator floor; live writes still PHP."),
        ...
        return new AspNetZeroPhpPathReport(
            ...
            [
                ...
                "Remaining CP ajax (catalogue/marketing/price-upload/portal integrations) + classic form POSTs; never invent cutoverAllowed.",
            ],
            Notes:
            [
