...
/// <summary>
/// Dry-run envelope for PHP <c>mfgr_wc_save</c> / <c>epc_mfgr_wc_save</c>
/// when <c>confirmWrites</c> is omitted. Live UPDATE/INSERT is
/// <c>IErpMfgrWcSaveWriteService</c>.
/// </summary>
public interface IErpMfgrWcSaveDryRun
{
    ErpMfgrWcSaveDryRunResult Evaluate(ErpMfgrWcSaveRequest request);
}
...
public sealed class ErpMfgrWcSaveDryRun : IErpMfgrWcSaveDryRun
{
    ...
    private static ErpMfgrWcSaveDryRunResult Refuse(string s, string c, string d, ErpMfgrWcSaveRequest r) =>
        new(s, 0, true, false, false, c, false, r.Id, r.Code, [], d, "/CP/content/shop/finance/erp/ajax_erp.php?action=mfgr_wc_save");
}

public sealed record ErpMfgrWcSaveRequest(long Id = 0, string? Code = null, bool ConfirmWrites = false);

public sealed record ErpMfgrWcSaveDryRunResult(
    ...
    IReadOnlyList<string> SimulatedSql, string Detail, string PhpAjax)
{
    public object ToPayload(object session) => new
    {
        ...
        intended = new { action = "mfgr_wc_save", id = Id, code = Code },
