...
public sealed class LegacyApiClientParityReporter : ILegacyApiClientParityReporter
{
    public LegacyApiClientParityReport BuildReport()
    {
        return new LegacyApiClientParityReport(
            ...
            [
                "On CloudPanel: diagnose_smoke_db.sh → apply_epc_api_clients_ddl.sh (clpctl) or use_php_dp_config_as_tenant_registry.sh → ensure_epc_api_clients_table.sh → issue_smoke_credentials.sh.",
