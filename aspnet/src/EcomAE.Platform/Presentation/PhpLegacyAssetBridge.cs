...
public static class PhpLegacyAssetBridge
{
    ...
    public static void Map(IEndpointRouteBuilder endpoints, IWebHostEnvironment env)
    {
        ...
        // Marketing screenshots / OG images — stack-neutral URLs (no .php in product HTML).
        endpoints.MapGet("/platform-assets/marketing_screens/{*filePath}", (string filePath) =>
        {
            ...
            var relative = Path.Combine("content", "general_pages", "marketing_screens", filePath.Replace('\\', '/'));
            var path = Path.GetFullPath(Path.Combine(repoRoot, relative));
            var root = Path.GetFullPath(Path.Combine(repoRoot, "content", "general_pages", "marketing_screens"));
