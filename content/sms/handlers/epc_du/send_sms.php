<?php
/**
 * du UAE — Business Messaging partner gateway.
 *
 * Fill api_url + api_key from your du Business / SMS partner contract.
 * sender_number defaults to +971567607011 (changeable in CP).
 */
...
list($row, $params) = epc_sms_load_operator($db_link, 'epc_du');
if (!$row) {
	epc_sms_exit_json(false, 'du operator not configured');
}
...
if ($apiUrl === '') {
	epc_sms_exit_json(false, 'du API URL is required (from your du Business / SMS partner contract)');
}
if ($apiKey === '') {
	epc_sms_exit_json(false, 'du API key is required');
}
if ($to === '' || $body === '') {
...
$msg = 'du send failed (HTTP ' . $res['http'] . ')';
