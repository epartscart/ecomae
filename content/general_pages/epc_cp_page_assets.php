...
function epc_cp_page_asset_url_map(): array
{
	...
	return array(
		...
		'shop/orders/oms-guide' => array(
			'css' => array(
				'/content/general_pages/epc_orders_cp_css.php?v=' . rawurlencode($ver . 'omsGuide1'),
			),
		),
		'shop/orders/orders' => array(
			...
			'js' => array(
				...
				'/lib/multiple_select/jquery.multiple.select.js',
				'/' . $backend . '/content/users/statistics/assets/main.js',
				'/' . $backend . '/content/shop/order_process/orders_config.php?v=' . rawurlencode($ver . 'omsGuide1')
					. (isset($_GET['order_id']) ? '&order_id=' . (int) $_GET['order_id'] : '')
					. (isset($_GET['status_id']) ? '&status_id=' . (int) $_GET['status_id'] : ''),
				'/' . $backend . '/content/shop/order_process/orders_cp.js?v=' . rawurlencode($ver . 'omsGuide1'),
				'/' . $backend . '/content/shop/order_process/epc_orders_fulfillment.js?v=' . rawurlencode($ver . 'omsGuide1'),
			),
		),
