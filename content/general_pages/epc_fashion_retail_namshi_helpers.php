<?php
/**
 * Fashion retail Namshi package — SEO, store URL, legacy chrome suppression.
 */
defined('_ASTEXE_') or die('No access');

function epc_fashion_retail_namshi_store_name(): string
{
...
function epc_fashion_retail_namshi_tagline(): string
{
	$settings = epc_portal_load_site_settings();
	if (!empty($settings['tagline'])) {
		return trim((string) $settings['tagline']);
	}
	return 'Fashion, beauty & lifestyle — UAE delivery, prices in AED';
}

function epc_fashion_retail_namshi_public_url(): string
{
	$settings = epc_portal_load_site_settings();
...
function epc_fashion_retail_namshi_apply_seo($DP_Content): void
{
	if (!is_object($DP_Content)) {
		return;
	}
	$name = epc_fashion_retail_namshi_store_name();
	$tagline = epc_fashion_retail_namshi_tagline();
	$bad = array('epartscart', 'eParts Cart', 'Autoparts', 'auto parts', 'autoparts', 'spare parts', 'Docpart');

	if (!empty($DP_Content->main_flag)) {
		$DP_Content->title_tag = $name . ' — Fashion & Beauty UAE';
		$DP_Content->description_tag = $name . ': ' . $tagline . ' Shop women, men, beauty & accessories with fast UAE delivery.';
		$DP_Content->keywords_tag = 'fashion UAE, beauty Dubai, makeup, skincare, dresses, shoes, AED, ' . $name;
	} else {
		...
		$DP_Content->keywords_tag = 'fashion, beauty, ' . $name . ', UAE, AED';
	}
}

function epc_fashion_retail_namshi_patch_template_html(string $html): string
{
	$title = epc_fashion_retail_namshi_store_name() . ' — Fashion & Beauty UAE';
	$desc = epc_fashion_retail_namshi_store_name() . ': ' . epc_fashion_retail_namshi_tagline();
	$keys = 'fashion UAE, beauty Dubai, makeup, skincare, dresses, shoes, AED, ' . epc_fashion_retail_namshi_store_name();
...
function epc_fashion_retail_namshi_scrub_legacy_strings(string $html): string
{
	$replacements = array(
		'eParts Cart (Autoparts)' => epc_fashion_retail_namshi_store_name(),
		'eParts Cart' => epc_fashion_retail_namshi_store_name(),
		'(Autoparts)' => '',
		'Autoparts' => 'Fashion',
		'auto parts' => 'fashion',
		'autoparts' => 'fashion',
