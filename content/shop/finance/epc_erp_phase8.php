...
/**
 * Drop a defence-in-depth .htaccess into an uploads directory so that even if a
 * script-type file lands there it can never be executed by the web server.
 */
function epc_erp_uploads_harden_dir($dir)
{
	...
	$rules = "# Auto-generated by ECOM AE — block execution of any uploaded code.\n"
		. "php_flag engine off\n"
		. "<IfModule mod_php.c>\nphp_flag engine off\n</IfModule>\n"
		. "<IfModule mod_php7.c>\nphp_flag engine off\n</IfModule>\n"
		. "<IfModule mod_php8.c>\nphp_flag engine off\n</IfModule>\n"
...
function epc_erp_document_upload(PDO $db, array $data, array $file)
{
	...
	if ($ext === '' || in_array($ext, $blockedExt, true) || !in_array($ext, $allowedExt, true)) {
		throw new Exception('File type not allowed: .' . ($ext === '' ? '(none)' : $ext) . '. Permitted: ' . implode(', ', $allowedExt));
	}
	...
	epc_erp_uploads_harden_dir($dir);
...
function epc_erp_phase8_next_no(PDO $db, $prefix, $table, $col)
{
	...
	// $table/$col are always supplied as internal literals; validate as plain
	// identifiers before interpolation as a defence-in-depth guard.
	if (!preg_match('/^[a-zA-Z0-9_]+$/', (string)$table) || !preg_match('/^[a-zA-Z0-9_]+$/', (string)$col)) {
		throw new Exception('Invalid sequence source');
	}
