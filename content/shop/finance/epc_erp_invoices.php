...
function epc_erp_invoice_print_html(array $doc): string
{
	...
	$html .= '<div class="hdr"><h1>TAX INVOICE</h1><p style="margin:4px 0 0;color:#64748b;">UAE e-invoice (PINT-AE) · ' . htmlspecialchars($doc['invoice_number'], ENT_QUOTES, 'UTF-8') . '</p></div>';
	$html .= '<div class="grid"><div class="box"><h3>Seller</h3><strong>' . $company . '</strong><br>TRN: ' . htmlspecialchars($seller['seller_trn'] ?? '—', ENT_QUOTES, 'UTF-8') . '<br>'
		. htmlspecialchars(($seller['seller_address_line1'] ?? '') . ', ' . ($seller['seller_city'] ?? ''), ENT_QUOTES, 'UTF-8') . '</div>';
	$html .= '<div class="box"><h3>Buyer</h3><strong>' . htmlspecialchars($buyer['buyer_name'] ?? '—', ENT_QUOTES, 'UTF-8') . '</strong><br>TRN: ' . htmlspecialchars($buyer['buyer_trn'] ?? '—', ENT_QUOTES, 'UTF-8') . '<br>'
		. htmlspecialchars(($buyer['buyer_address_line1'] ?? '') . ', ' . ($buyer['buyer_city'] ?? ''), ENT_QUOTES, 'UTF-8') . '</div>';
	$html .= '<div class="box"><h3>Invoice</h3>Date: ' . date('Y-m-d', (int)$doc['issue_date']) . '<br>Due: ' . date('Y-m-d', (int)$doc['payment_due_date'])
	...
	$html .= '<div class="foot">Generated by ECOM AE Blockchain BOS Enterprise · UUID ' . htmlspecialchars($doc['uuid'], ENT_QUOTES, 'UTF-8') . '</div></body></html>';
	return $html;
}
