...
$rate = isset($how_get_json['delivery_price']) ? (float)$how_get_json['delivery_price']
	: (isset($how_get_json['rate']) ? (float)$how_get_json['rate']
	: epc_channel_demo_rate($carrier, isset($how_get_json['weight_kg']) ? (float)$how_get_json['weight_kg'] : 1, isset($how_get_json['country']) ? $how_get_json['country'] : 'AE'));
?>
<p class="lead">Delivery — <?php echo epc_channel_h(translate_str_by_id($obtain_mode['caption'])); ?></p>
<table class="table">
<tr><th>Carrier &amp; service</th></tr>
<tr><td><?php echo epc_channel_h($cname); ?> — <?php echo epc_channel_h($sname); ?> (customer pays <?php echo epc_channel_money($rate); ?> AED + VAT if UAE)</td></tr>
