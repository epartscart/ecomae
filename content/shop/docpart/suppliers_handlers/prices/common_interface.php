		...
		*/
		...
		if(!empty($analogs_groups)) {
		    foreach($analogs_groups as $analogs_group => $analogs) {
				...
				list($office_storage_dataInfo, $price_ids_for_query) = epc_prices_build_office_storage_data_info(
					$db_link,
				...
				{
				//Формируем запрос (indexed article_search + narrow columns)
				$SQL = 'SELECT `manufacturer`, `article_show`, `name`, `exist`, `price`, `time_to_exe`, `storage`, `min_order`, `price_id`
					FROM `shop_docpart_prices_data`
					WHERE ('.$SQL_WHERE.') AND `price_id` IN (' . implode(',', array_map('intval', array_unique($price_ids_for_query))) . ')';
