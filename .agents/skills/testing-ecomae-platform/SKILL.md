...
1. **Sub-industry cards** — Each industry has 10-17 sub-industries in a grid layout. Cards show: photo, name, description, ERP/CP/Store badges, 5-step process flow (Discovery → Configure → Deploy → Operate → Optimize), 3 sample products with images/names/prices, CTA buttons.

2. **Filter buttons** — "All", "Core(N)", "Extended(N)", "Specialized(N)" buttons toggle card visibility via `display: none` on elements with `data-group` attribute.
...
Expected: Each returns HTTP 200 with ~44-88KB response (NOT the generic 228KB homepage).
