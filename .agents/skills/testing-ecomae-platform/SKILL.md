...
1. **Sales orders seed into lines only (no headers):** `epc_jw_seed_sample_data()` inserts into `epc_erp_sales_order_lines` with `order_id = 0`. The sales orders list page queries headers, which don't exist. Fix: create sales order headers first, then link lines.
2. **Repairs may partially fail silently:** The seed function uses `catch (Throwable $e) {}` which swallows all errors. Only 1 of 3 repairs may seed successfully. To debug, temporarily add error logging in the catch block.
3. **Trial balance may show empty:** GL entries are posted via `epc_jw_post_weight_ledger()` but the trial balance tab rendering might have a bug or the weight ledger table might not be created. Check if `epc_jewel_weight_ledger` table exists.
4. **Suppliers/customers have no dedicated list page:** Seeded into `epc_erp_contacts` table but no URL route to view them as a list (vendors URL redirects to dashboard).

### Verifying Seed Results
```
Dashboard: ?area=overview&tab=dashboard → Industry = "Jewellery & watches", Inventory value > 0
Inventory: ?area=inventory_mgmt&tab=inventory → 15 items with Metal/Karat/Weight columns
POs: ?area=procurement&tab=purchase_orders → 5 POs with Karat/Weight/Rate columns
SOs: ?area=sales&tab=sales_orders → Currently 0 (bug: no headers)
Repairs: ?area=jewellery&tab=jw_repairs → Should be 3, may only show 1
Trial Balance: ?area=jewellery&tab=jw_trial_balance → May be empty (rendering bug)
```
...
### Current State (may need fixing)
- "Jewellery" appears as a separate sidebar module entry (last of 36 modules)
- Has its own area=jewellery with sub-tabs (hub, trial balance, repairs, sample data)
- This violates the user's explicit requirement

### Correct Architecture
- Jewellery-specific columns (Metal, Karat, Weight, Purity, etc.) should appear conditionally in existing modules when `erp_industry_profile = 'jewellery'`
- Dual Trial Balance (weight + value) should be under General Ledger, not separate Jewellery area
- Repairs should be under Service Management
- Master data (Karat, Rate Type, etc.) should be under Inventory Management or System Administration
- The separate "Jewellery" sidebar entry should be removed
