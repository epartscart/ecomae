...
## Testing Industry Storefronts (28 Industries)

### Local PHP CLI Server
The industry storefronts can be tested locally without a live server:
```bash
cd /home/ubuntu/repos/ecomae
php -S localhost:8088 content/general_pages/industry_templates/router.php
```

Then navigate to `http://localhost:8088/{industry}` where `{industry}` is one of:
- healthcare, food_beverage, construction, automotive, jewellery, electronics, fashion, manufacturing, education, hospitality, beauty, retail, agriculture, logistics, energy, finance, technology, media, sports, nonprofit, wholesale, rental, cleaning, pet, printing, security, homeliving, professional

### Key Test Areas for Industry Storefronts

1. **Sub-industry cards** — Each industry has 24-29 sub-industries (722 total across 28 industries) in a grid layout. Cards show: photo (Unsplash URL), name, description, ERP/CP/Store badges, 5-step process flow (Discovery → Configure → Deploy → Operate → Optimize), 3 sample products with images/names/AED prices, CTA buttons (Try Store + Open CP).

2. **Filter buttons** — "All(N)", "Core(N)", "Extended(N)", "Specialized(N)" buttons toggle card visibility via `display: none` on elements with `data-group` attribute. Uses `array_chunk($subIndustries, ceil(count/3))` for 3-group division. Verify via console: `document.querySelectorAll('.sub-card[style*="display: none"]').length` after clicking a filter.

3. **Cart functionality** — Add to cart buttons on product cards update header badge count, cart panel shows items with quantity +/- controls and recalculated total. Uses localStorage for persistence.

4. **Auth modal** — Register tab has: Full Name, Email, Phone, Company Name, Industry dropdown (ALL 28 industries), Password. Login tab has email/password. Modal shows industry context indicator.

5. **Cross-industry differentiation** — Each industry has distinct: color scheme (CSS --primary variable), hero icon (Font Awesome), background photo (Unsplash), stats, product names, ERP feature list. Verify industries look visually different.

6. **Mobile responsiveness** — At 375-400px width: nav items hidden, header compact, stats flow to 2-column, cards stack single column, no horizontal overflow.

### Testing Sub-Industry Data Integrity
After expanding sub-industries, verify data integrity across all 28 files:
```bash
cd /home/ubuntu/repos/ecomae/content/general_pages/industry_templates
php -r "
\$dir = __DIR__;
\$total = 0; \$mismatches = 0;
foreach(glob(\"\$dir/*.php\") as \$f) {
    if(basename(\$f)==='_base_template.php' || basename(\$f)==='router.php') continue;
    \$code = file_get_contents(\$f);
    preg_match('/\\\\\\$subIndustries\\s*=\\s*array\\(([^;]+)\\);/', \$code, \$m);
    \$subs = \$m[1] ?? '';
    \$subCount = substr_count(\$subs, \"'\") / 2;
    preg_match_all('/=>\\s*array\\(\\s*\\'photo\\'/', \$code, \$pm);
    \$prodCount = count(\$pm[0]);
    if(abs(\$subCount - \$prodCount) > 1) { echo basename(\$f).\" MISMATCH: \$subCount vs \$prodCount\\n\"; \$mismatches++; }
    \$total += \$subCount;
}
echo \"Total: \$total sub-industries, \$mismatches mismatches\\n\";
"
```
Expected: 722 total, 0 mismatches.

### Testing Sub-Industry Rendering (Local PHP Server)
Use the test harness for quick visual verification:
```bash
cd /home/ubuntu/repos/ecomae
php -S localhost:8080 &
# Then navigate to: http://localhost:8080/test_industry.php?i=healthcare
```
The `test_industry.php` file loads `_base_template.php` with a specific industry's data. Check:
- Card count matches the `$subIndustries` array length
- Photos load (not gradient fallback — look for `<img>` tags vs `linear-gradient` divs)
- Product tiles show 3 items with AED prices
- Filter buttons divide cards into 3 groups (ceil(N/3) per group)

### Live Site Verification
After PR deployment, verify subdomains serve the correct template:
```bash
for industry in food construction healthcare automotive jewellery; do
  curl -sk -o /dev/null -w "$industry.ecomae.com: HTTP %{http_code} | %{size_download}B\n" "https://$industry.ecomae.com" --max-time 15
done
```

Expected: Each returns HTTP 200 with ~85-90KB response (NOT the generic 228KB homepage).

### Live Client Sites Section
PR #128 adds a "Live Client Sites" animated showcase to `/platform/industries` and ecomae.com homepage. This shows 5 live tenant cards (epartscart, electronicae, stylenlook, thejewellerytrend, taxofinca) with colored borders, pulse animations, and hover lift effects. These sections require PR deployment to verify on production.

### Base Template (`_base_template.php`)
All 28 industry templates share a base template that provides:
- Professional header (logo, nav menu, cart icon, Login/Register buttons)
- Cart panel (slide-out panel with item list, quantity controls, total)
- Auth modal (Login/Register tabs with 28-industry dropdown)
- Hero section with particle animation and gradient text
- Gallery with hover effects (translateY, box-shadow, image scale)
- Footer with quick links, social icons, newsletter signup, demo credentials
